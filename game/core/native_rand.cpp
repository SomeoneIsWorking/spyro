...
// then super-calls the recompiled body, so the guest code still does the work. That is instrumentation,
// not a port. This is a real replacement — the recompiled body never runs once this is installed —
// chosen to be small and exactly specified so the REPLACEMENT MECHANISM can be proven before it is
// pointed at anything load-bearing.
//
...
// pick its idle animation (C071), so a different sequence changes observable behaviour and would make
// any future frame-for-frame comparison drift for reasons that have nothing to do with the code under
// test.
//
...
void rand_native(Core* c) {
  ...
  // The 64-bit product is what `mult` leaves in hi/lo; the low half is what the LCG uses. Compute it
  // signed, because MIPS `mult` is a SIGNED multiply and the substrate models it that way — using an
  ...
  // per-call differential flagged the mismatch on call #10 — the first call where the caller happened
  // to leave a different value in $at. Architecturally $at is the assembler temporary and no
  // compiler-generated code reads it across a call, so this is harmless in practice. It is reproduced
  // anyway because "harmless" is a judgement and "identical" is a measurement, and the moment a
  // replacement is allowed to differ "where it does not matter" the differential stops meaning
  // anything. I would not have noticed this by reading the code.
  c->r[1] = 0x80070000u;
}
...
void rand_owned(Core* c) { ndiff_run(c, "rand@0x8006272C", rand_native, gen_func_8006272C); }
