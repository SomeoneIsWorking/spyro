...
// link standalone). It reaches the recompiled game through psxport_recomp()->field instead, and
// this file is the ONE place those generated symbols are named — game code referring to generated
// code, which is fine; the coupling the seam breaks is framework→generated.
#include "core.h"
...
static const RecompRegistry g_spyro_recomp = {
    ...
    // Spyro has NO code overlays — one executable, all data in WAD.WAD. emit.py still emits an
    // (empty) overlay table, so these stay wired to it and g_rec_overlay_count is 0; overlay_router
    // simply never matches an address.
    /* overlays             */ g_rec_overlays,
    /* overlay_count        */ g_rec_overlay_count,
    /* shard_set_override   */ shard_set_override,
    // Tomba!2-shaped members: setters for ITS overlay modules. Spyro emits no overlay modules, so
    // there is no such symbol to point at. (That these are named per-overlay in a game-agnostic
    // struct is a residual framework wart — see docs/codemap.md.)
...
void spyro_install_recomp() {
  psxport_install_recomp(&g_spyro_recomp);
}
