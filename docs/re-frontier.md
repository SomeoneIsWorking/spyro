...
- evidence: C158
- where: game/core/game_hooks.cpp (future `fps60ReadSceneCam` hook); external/psxport/runtime/recomp/game_hooks.h + game_hooks_opt.cpp + fps60.cpp (framework seam)
- gap: THE FRAMEWORK DEFECT IS FIXED at psxport `a1c53d7c`: `Fps60::sceneCam` now requires the game's `fps60ReadSceneCam` hook and aborts if it is absent, rather than interpreting Tomba!2 scratchpad offsets as a universal camera layout. Spyro still has no caller of `sceneCam` and no native producer using `RenderQueue::drawWorldQuad`, so the missing hook is inert today. Before the first such producer lands, derive Spyro's view matrix and translation from the game's own camera state and supply them through the hook. Projection constants are already correct: `ProjParams` receives Spyro's SetGeomOffset/SetGeomScreen values in `game_config.cpp`.
