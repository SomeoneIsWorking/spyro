...
| Natively OWNED guest code | `game/core/native_rand.cpp`, `game/core/native_leaf.cpp`, `tools/own_candidates.py` | 🟡 | **The honest ratio: 20 observation wrappers to 2 native bodies** (C075). Most overrides super-call the recompiled body; the CD/pad ones are platform-level *supply*. Genuinely owned: the vblank wait, `rand()` `0x8006272C`, and three hot leaves — `copy3` `0x80017700` (136 callers), `fill` `0x80016914` (59), `zero3` `0x800176F0` (40) — **276 static call sites**, each byte-exact on 25 consecutive calls (C076). Every native body is gated byte-exact against the body it replaced on each gate run (`PSXPORT_NDIFF`, I019) — validated in both directions, and it caught a real `$at` inequivalence in the first replacement that reading the code did not. Pick targets with `tools/own_candidates.py` (I020), not by eye — that already went wrong once: `0x8001ED5C` looks like a small buffer flip and is the whole per-frame stage dispatcher. Next: frontier `own.next-targets`. |
