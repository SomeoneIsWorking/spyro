...
// dropping in on an ease table, the logo sitting with a pulsing strip under it, the logo sliding out,
// and a mirrored banner pair. Every one of them is ONE call to the guest's sprite emitter
...
constexpr uint32_t kGateVarPtr = 0x80075680u; // *(*this) — a global counter the logo drop-in waits on.
...
//   drop-in  0x8006FA74: 0,8,16,25,34,44,54,65,76,88,101,114,128,123,120,126  (ease-out + overshoot)
//   slide-out 0x8006FA84: 127,125,122,118,113,107,100,92,83,73,62,50,37,23,9,0
//   banner   0x8006FA64: 60,2,1,128, 40,2,1,128, 20,2,1,128, 8,2,1,128
...
// id*8` for the four u8s) and confirmed by dumping all 64: ids 0..63 are coherent (three 255x15 text
// pages at tpage 0x1C/1D/1E, ten 16x16 glyphs at 0x1A, the 255x128 logo at 0x98), and id 64 is
...
// Dumped: 2C808080 (neutral), 2C00F0F0 (yellow), 2CC0C0C0 (bright), 2C404040 (dim). The code byte is
// 0x2C on all four — POLY_FT4, textured, MODULATED (bit 0 clear = not raw), opaque (bit 1 clear).
...
struct MissingLayer { uint32_t fn; const char* what; };
...
struct SpriteRec {
  ...
  static SpriteRec read(Core* c, uint32_t id) {
    ...
    return { c->mem_r16(b + 0), c->mem_r16(b + 2),
             c->mem_r8(b + 4), c->mem_r8(b + 5), c->mem_r8(b + 6), c->mem_r8(b + 7) };
  }
};
...
struct MenuState {
  ...
  static MenuState read(Core* c) {
    ...
    return { c->mem_r32(kMode), c->mem_r32(kPage), c->mem_r32(kAnim),
             gatePtr != 0 && c->mem_r32(gatePtr) >= kGateValue };
  }
};

// The arms of mode 0, as the guest's if-chain orders them. `kNone` is a real answer: the chain draws
// no sprites for a page it does not name, and the frame is then the 3D backdrop alone.
enum class Arm { kNone, kLogoDropIn, kLogoHold, kLogoSlideOut, kBanner };
const char* armName(Arm a) {
  switch (a) {
    case Arm::kLogoDropIn:   return "page 2: logo DROPPING IN along the ease table at 0x8006FA74";
    case Arm::kLogoHold:     return "page 3: logo held + the pulsing strip under it";
    case Arm::kLogoSlideOut: return "page 4: logo SLIDING OUT along the ease table at 0x8006FA84";
    case Arm::kBanner:       return "page 4: the mirrored banner pair";
...
bool SpyroRenderer::spriteEmit(int32_t x, int32_t y, int32_t id, uint32_t style,
  ...
  if (sid >= kSpriteCount || style >= kStyleCount) {
    ...
    lucent::warn("titlefx", "sprite id {} / style {} is outside the tables (ids 0..{}, styles 0..{}) "
                            "— NOTHING emitted for this call",
                 id, style, kSpriteCount - 1, kStyleCount - 1);
    return false;
  }
  ...
  for (int i = 0; i < 4; i++) { xs[i] += drawOfsX; ys[i] += drawOfsY; }
  ...
  c->game->rq.push2dQuad(RQ_HUD, /*order_2d_fg=*/1, xs, ys, us, vs, rs, gs, bs,
                         ...
                         /*clut_x=*/(int)(s.clut & 0x3Fu) * 16, /*clut_y=*/(int)((s.clut >> 6) & 0x1FFu),
                         /*tw_mx=*/0, /*tw_my=*/0, /*tw_ox=*/0, /*tw_oy=*/0,
                         clipX0, clipY0, clipX1, clipY1, semi);
  return true;
}
...
bool SpyroRenderer::titleMenuRender(int32_t drawOfsX, int32_t drawOfsY,
                                    int32_t clipX0, int32_t clipY0,
                                    int32_t clipX1, int32_t clipY1) const {
  Core* c = mC;
  const MenuState st = MenuState::read(c);
  if (st.mode != 0) {
    lucent::error("render", "  stage 13 mode [0x{:08X}] = {} has NO native producer. Only mode 0 (the "
  ...
  if (st.page == 2) {
    ...
    if (st.gateOpen) {
      if (st.anim >= kEaseLen) {
        lucent::warn("titlefx", "drop-in anim index {} is past the {}-entry ease table at 0x{:08X}; "
                                "the guest reads it unchecked, so this port declines rather than "
                                "reproduce an out-of-table read. NOTHING emitted.",
                     st.anim, kEaseLen, kEaseDropIn);
      } else {
        const int32_t y = (int32_t)c->mem_r8(kEaseDropIn + st.anim) - kLogoYBias;
        emitted += spriteEmit(kLogoX, y, kIdLogo, kStyleNeutral, drawOfsX, drawOfsY,
                              clipX0, clipY0, clipX1, clipY1) ? 1 : 0;
      }
    }
  } else if (st.page == 3) {
    arm = Arm::kLogoHold;
    emitted += spriteEmit(kLogoX, 0, kIdLogo, kStyleNeutral, drawOfsX, drawOfsY,
                          clipX0, clipY0, clipX1, clipY1) ? 1 : 0;
    const uint32_t stripStyle = ((st.anim & kBlinkMask) < kBlinkHalf) ? kStyleBright : kStyleNeutral;
    emitted += spriteEmit(kStripX, kStripY, kIdPressStrip, stripStyle, drawOfsX, drawOfsY,
                          clipX0, clipY0, clipX1, clipY1) ? 1 : 0;
  } else if (st.page == 4) {
    if (st.anim < kAnimBannerFrom) {
      ...
      if (st.anim >= kEaseLen * 4u) {   // the banner table is 16 4-byte groups; see the dump above
        lucent::warn("titlefx", "banner anim index {} is past the table at 0x{:08X} — NOTHING emitted.",
                     st.anim, kEaseBanner);
      } else {
        const int32_t y = (int32_t)c->mem_r8(kEaseBanner + st.anim) - kBannerYBias;
        emitted += spriteEmit(kBannerXLeft, y, kIdBanner, kStyleNeutral, drawOfsX, drawOfsY,
                              clipX0, clipY0, clipX1, clipY1) ? 1 : 0;
        emitted += spriteEmit(kBannerXRight, y, -kIdBanner, kStyleNeutral, drawOfsX, drawOfsY,
                              clipX0, clipY0, clipX1, clipY1) ? 1 : 0;
      }
    }
  }
  ...
  // the same as "no arm".
  lucent::debug("titlefx", "mode={} page={} anim={} gate={} arm=[{}] emitted={} | world layers "
                           "un-produced: {}",
                st.mode, st.page, st.anim, st.gateOpen ? "open" : "shut", armName(arm), emitted,
...
void SpyroRenderer::titleMenuBacklogReport() const {
  lucent::warn("render", "stage 13 native leg is PARTIAL: the front-end sprite layer (guest "
