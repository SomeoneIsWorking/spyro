...
// false = the native path, true = the PSX recomp reference. SBS and dualcore SET it that way when
// they configure their two cores (sbs.cpp M_RENDER: core A native, core B PSX), so a game that
// inverted the sense would silently give an SBS core configured for "native" the reference picture
// — the wrong-source failure this project keeps paying for. Hence: no PSXPORT_SPYRO_* render
// switch; `PSXPORT_RENDER_PSX=1` selects the reference, exactly as it does for Tomba!2.
#pragma once
