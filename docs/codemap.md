...
| Renderer — VISUALLY CONFIRMED | `external/psxport/runtime/recomp/gpu_vk.cpp` | 🟡 | **The port renders correctly** (C094): the Insomniac logo over Spyro's 3D landscape, 512x240, 93.3% non-black, 2126 colours, captured headless. Capture it yourself: `PSXPORT_REPL=1`, then `shotregion <path> <x> <y> <w> <h>`. **Content lives in ONE of the two buffers** — draw0 at y=8, draw1 at y=248 (C068) — so dumping the wrong region gives an all-black image that looks like a broken renderer (C095). That is what made issue 0018 unanswerable for months. |
