...
- notes: RenderWorldChunks is structurally mapped (C198) AND its per-call depth ORACLE is live (C201, PSXPORT_WORLD_CENSUS=1): a reference-leg run captured per-call SZ3 depth range 4..12806 (mean 7805) from the scratchpad (0x1F8002AC, 2-byte stride) plus 358,830 faces across 802 OT bins (depth>>7, bins 2..806, smooth near-to-far gradient) and 688,938 clean pool packets (329,840 quads + 359,098 tris, 0 malformed, 0 unparsed, no depth word in the body — confirming C198 at runtime). The input census (2026-08-17) measured 2088 live calls (1555 flat-list a0<0, 533 occlusion-group a0>=0), 169,495 flat entries (max 109) + 72,879 occ entries (max 178), 40,684,452 pool bytes, zero empty calls. Next: BEGIN THE ORACLE — diff the world renderer's per-call depth/geometry against the framework's native render, and start byte-exact ownership for DEPTH (the 2D/3D discriminator). Clip-bound immediates 0x10000/0x1000000/0x2000000 at 8 sites.
