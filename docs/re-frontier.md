...
- where: game/core/cd_queue.cpp `lp_800127C0`; titles/spyro1/core/spyro1_field_scheduler.cpp `serviceSkipMap`; `PSXPORT_DEBUG=skipmap`
- gap: Boot logos have no guest Start path and no recovered complete title-owned cancellation/transition route, so they are not skippable. Title/attract keeps its legitimate guest transition. Stage mode 14 is now classified as recorded/demo playback and ALREADY handles Start/Cross in guest `0x800331AC` by accelerating its cursor toward the natural completion writer `0x8002D440`; adding a native transition would duplicate it. The title-card and level-transition helpers that wrote guest timer/state values are removed. Observed stage13/sub3 phases are required streaming/I/O, not a presentation hold, and remain unmodified. NEXT: recover a complete title-owned cancellation/transition route for a presentation-only surface; never fast-forward simulation, advance a timer, or write a phase/scene pointer. Never pulse Start across modes 0/2 gameplay, where it means UI/pause.
