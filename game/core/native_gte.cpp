...
// leading-zero counter to normalise it, index the table at 0x80074B84 and shift back. The two bodies
// differ ONLY in which registers hold the input and the shift amount (veclen: at/a0; isqrt: a0/at),
// so the arithmetic is factored and each caller places the results in its own registers. Duplicating
// it would mean a fix to one and not the other, which is the likelier future bug.
struct SqrtTail { uint32_t v0, a1, shift, a2, a3, t0; };
...
void isqrt_native(Core* c) {
  ...
  if (val == 0) { c->r[8] = 0; c->r[2] = 0; return; }  // t0 = 0 (delay slot), v0 = t0 >> 12
...
void vscale_native(Core* c) {
  ...
  c->mem_w32(p + 4, r2);
  c->mem_w32(p + 8, r3);
  c->r[1] = r1; c->r[2] = r2; c->r[3] = r3;
}

void veclen_owned(Core* c) { ndiff_run(c, "veclen@0x800171FC", veclen_native, gen_func_800171FC); }
void vscale_owned(Core* c) { ndiff_run(c, "vscale@0x800175B8", vscale_native, gen_func_800175B8); }
void mvmva_owned(Core* c)  { ndiff_run(c, "mvmva@0x80017048",  mvmva_native,  gen_func_80017048); }
void isqrt_owned(Core* c)  { ndiff_run(c, "isqrt@0x80017A38",  isqrt_native,  gen_func_80017A38); }
