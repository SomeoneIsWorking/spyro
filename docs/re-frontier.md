...
- NEXT, in the abort's own order: (a) add normal face emission and RenderQueue ownership for 0x80023AC4; dynamically reach and verify the separate alternate/status-plane clipping arm before calling the producer complete (resolved XYZ, SXY/SZ, normal packet content, numeric bins and touched global splice words are exact); (b) stage 13 modes 1 and 2 of 0x8007CEE4 (page-driven text screens, 3-slot save screen) — RE'd but never reached by a drivable run; (c) the 3D backdrop shared by stage 13 and the FIELD: 0x800521C0, 0x8001F158, 0x8001F798, 0x800258F0, 0x8004EBA8.
