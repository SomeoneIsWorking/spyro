...
- gap: BLOCKED on a keying decision, not on more RE (docs/issues/0013). C032: all 37 overlays share ONE staging arena at 0x8007AA38 (read-only constant [0x800113A0]); a0 to the loader is the overlay INDEX (0x25=37 at site 0x8001253C). psxport keys overlays by load address, so a second entry in overlay_bases would collide with OVL0 rather than add an overlay. Next action is to READ psxport's RecompRegistry keying and decide whether an index-qualified key is a small change or a redesign — do not guess which. OVL0 alone is recompiled and the gate is green on it. Static call-site argument recovery is now a tool: tools/callsite_args.py (I006).
