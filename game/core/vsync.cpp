...
// via the helper at 0x8005DD0C: `while ([0x800749E0] < a0) spin;` with an (a1<<15)-iteration
// budget, printing "VSync: timeout" when it runs out. On real hardware that counter is incremented
// by the VSync IRQ callback. This runtime raises no IRQs, so the counter is frozen at its initial
// value and EVERY VSync times out — which is what the boot log showed.
//
...
// ── The VBlank callback
// ──────────────────────────────────────────────────────────────────────────── The boot's input
// setup at 0x800123C8 does three things in a row, and the third is the one that matters here:
...
// VBLANK INTERRUPT CALLBACK, and every subsequent invocation is an IRQ. This runtime raises no
// IRQs, so the decoder ran exactly once at boot and never again (measured: probe on 0x80053C68 logs
// "call #1" and nothing else in a 20s run). With it dead, the pad-class word [0x80077384] stays 0,
// the decoder's "no controller" arm is the only one ever taken, and the only thing that ever
// publishes input is the attract demo's playback path (C062) — which is why the port loops attract
// forever.
//
// So the fix is the same shape as the counter above: supply what the missing IRQ would have
// supplied. We do NOT reimplement the decoder — we run the guest's own registered callback body,
// once per vblank, which is exactly when the console would have run it.
//
// 0x8005DE58 is libetc's VSyncCallback(fn): it takes the handler in a0 and hands it to the
// interrupt callback table via [0x800749AC]+20 with a0=4 (the VSYNC slot). Intercepting it here —
// rather than hardcoding 0x80053C68 — means the port follows whatever the game registers, including
// a later re-registration, and it fails visibly (no callback recorded) rather than silently running
// a stale address if the boot path ever changes.
...
void vsync_callback_set(Core *c) {
  ...
  if (fn != g_vblank_cb) {
    g_vblank_cb = fn;
    lucent::info("vsync",
                 "VSyncCallback(0x{:08X}) registered — this is the per-vblank handler the "
                 "port must run, since no IRQ will.",
                 fn);
  }
...
// Deliver the vblank the way the console does: run the guest's VBlank ROOT handler, not one
// hand-picked callback.
//
// WHY THIS CHANGED. This used to call only the handler captured from VSyncCallback — libetc
// callback SLOT 4. But slot 4 is one of EIGHT, and the game registers others: Spyro puts 0x80067CD4
// in slot 7 at frame 835, and that handler is the sole setter of the flag its title screen polls
// every frame. With only slot 4 delivered, slot 7 never ran, so the title screen could never
// advance (issue 0027, C117). Special-casing one callback meant silently dropping every other one
// the game registers.
//
...
//
// THE ROOT HANDLER ALSO OWNS THE VBLANK COUNTER ([0x800749E0]) — it increments it on entry. So when
// it runs, the port must NOT also maintain that counter; vblank_wait re-reads it instead. Returns
// true if the guest's handler ran, so the caller knows who owns the count this iteration.
//
// The register file is saved and restored around the call for the same reason as before: this runs
// from INSIDE a guest call, and a real IRQ preserves the interrupted function's registers.
//
// ── AND THE HANDLER DOES NOT RUN ON THE INTERRUPTED STACK
// ──────────────────────────────────────────
//
// $sp IS NOT A STACK POINTER ON A PSX AT AN ARBITRARY INSTRUCTION. The kernel's exception entry
// saves the interrupted context and switches to its own stack before running the interrupt chain,
// so a PS1 function may legally keep scratch values in $sp/$gp/$fp, and hand-written renderers do —
// they are three more registers. Spyro's is one, and this is from the recompiled instructions, not
// inference: `gen_func_800258F0` (RenderWorldChunks, generated/shard_3.c) saves sp/gp/fp in its
// prologue, then writes `sp = -1`, `sp = 0x1F800000` (the scratchpad base) and `sp = r1 + 7680` in
// its inner loops, and reloads all three from its frame at the end.
//
// MEASURED CONSEQUENCE of getting this wrong, because it does not fail as a stack bug. With the
// host turn armed and the handler run on `c->r[29]`, the port died in RenderWorldChunks on an
// UNMAPPED read8 at 0x9006E9AB about half a second into the run. `PSXPORT_DEBUG=hostturn` showed
// why: turns 25-31 were taken at libetc function entries with sp=0x801FFFxx (a real stack), turn 32
// was taken at a loop back-edge inside the renderer with sp=0x80071B00 and gp=0x80071D20 —
// mid-table addresses, not a stack — and the handler chain wrote its frames there, over libetc's
// data. The crash then surfaced somewhere else entirely, reading a pointer that had been
// overwritten.
//
...
// (hle.cpp `HLE_WORK_BASE` 0x8000E000 / `HLE_B0TABLE` 0x8000F000 / `HLE_C0TABLE` 0x8000F800). So
// this is the framework's existing convention rather than a new claim on guest memory.
//
...
// psxport's own work area words at 0x8000F16C / 0x8000F800-0x8000F807. Nothing else in the region
// is ever written. The stack therefore sits directly below `HLE_WORK_BASE` and grows down, with the
...
// measurement and "did it run off the bottom" is a test rather than a hope. Any value works; this
// one is recognisable in a RAM dump and is not a plausible pointer or count.
...
void handler_stack_init(Core *c) {
  for (uint32_t a = kHandlerStackFloor; a < kHandlerStackTop; a += 4) {
    c->mem_w32(a, kStackPoison);
  }
  lucent::info("vsync",
               "vblank handler stack armed at [0x{:08X},0x{:08X}) — the guest's interrupt "
               "handler does NOT run on the interrupted $sp (see vsync.cpp)",
               kHandlerStackFloor,
               kHandlerStackTop);
}

// How far down the handler chain has ever reached, as an address. Reported by
// `PSXPORT_DEBUG=vsync`; the floor word is checked unconditionally, because an overflow silently
// corrupts whatever is below and would otherwise present as a bug somewhere else entirely — which
// is exactly the failure this whole comment is about.
uint32_t handler_stack_low_water(Core *c) {
  for (uint32_t a = kHandlerStackFloor; a < kHandlerStackTop; a += 4) {
    if (c->mem_r32(a) != kStackPoison) {
      return a;
    }
  }
...
bool run_vblank_callback(Core *c) {
  ...
  *static_cast<R3000 *>(c) = saved; // …and the context restore its exit performs
  ...
  if (c->mem_r32(kHandlerStackFloor) != kStackPoison) {
    lucent::error("vsync",
                  "the vblank handler chain ran off the bottom of its {}-byte stack at "
  ...
  if (ch_vsync) { // guards the SCAN, not the print
    ...
    if (lw < deepest) {
      deepest = lw;
      lucent::debug(ch_vsync,
                    "vblank handler stack peak: {} bytes used (low water 0x{:08X} of "
                    "[0x{:08X},0x{:08X}))",
                    kHandlerStackTop - lw,
                    lw,
                    kHandlerStackFloor,
...
// sleeps exactly `quota/60 s` per call on the strength of that declaration (gpu_native.cpp
...
// themselves — `lucent::debug` does not evaluate its arguments when the channel is off, so a
// counter bumped inside the log call would only count while someone was watching, which is the
// classic instrument that cannot report its own denominator.
//
...
// vblank rate is it. The guest's DrawOTag reaches the GPU through DMA2, and the framework drains
// the queue at the END OF THAT WALK (gpu_native.cpp `gpu_dma2_linked_list` -> rq.flush), so by the
// time this loop flushes, the queue is already consumed and this boundary cannot see a draw happen.
// The rate of NEW rendered frames is `rebuild_geom` from `PSXPORT_DEBUG=presentskip`, which is a
...
unsigned long g_pace_ack = 0;     // explicit host-turn deadline acknowledgements
// Fields where THIS file was the queue's first consumer (n>0 and not yet consumed). Expected 0
// for this port, per the paragraph above — it is the instrument's own check that the sample point
// is downstream of the DMA2 flush, so a NON-zero value is the interesting answer and means a second
...
// ──────────────────────────────────────────────────────────────
//
// Everything the console's VBlank does, in the order it does it. There are TWO callers and they
// must not be two implementations of "a field happened":
//
//   * `vblank_wait` below — the guest reached a VSync wait and is asking for fields;
//   * `spyro_host_turn` — the guest is in a loop that waits for a field WITHOUT calling anything
//   the
...
// at 0x80067628 blocks in a bare spin (`generated/shard_4.c` L_80067684: `v0 = [0x80075B58]; beq
// v0, zero, back` — nothing else in the loop), and the SOLE writer of that flag is the card
// driver's completion callback 0x80067CD4, which the game installed as libetc VBlank callback
// slot 7. That spin calls no library function, so before this the port had no way to deliver the
// field the flag depends on and the run wedged with no frame ever presented again — the user's
// "SELECT MEMORY CARD softlock". The spin is not special: it is one member of a class (the card
// READ and WRITE op state machines have the same shape), so the fix is the framework's class-wide
// mechanism rather than an override on that one address. `rec_host_turn_register` is one line; see
// host_turn.cpp for why its arming interval is a hardware fact and not a tuned constant.
//
...
bool deliver_field(
    Core *c, const char *site, bool presentHere, bool paceHere, bool acknowledgeHostTurn) {
  ...
  if (g_in_field) {
    ++g_field_refused;
    lucent::debug("pace",
                  "field refused (site={}): one is already in flight — a vblank handler is "
                  "not re-entrant on hardware either. refused={} so far",
                  site,
                  g_field_refused);
    return false;
  }
  ...
  if (skip == BootSkipAction::Baseline) {
    ...
    lucent::info("bootskip",
                 "fresh Start edge: presentation clock {} -> {} (+{}); guest boot "
  ...
  // `start_edges=0` means we scanned and found none.  State changes and every edge are never
  // capped.
  {
    constexpr uint16_t kPadStart =
        0x0008u; // host/PSX active-low bit; guest decoder publishes 0x800
    ...
    if (start_edge || changed) {
      lucent::debug("skipmap",
                    "field={} start_edge={} region={} boot_phase={} stage={}/{}/{} "
                    "edges={}",
                    fields,
                    start_edge ? 1 : 0,
                    boot ? "boot" : "stage",
    ...
    if (fields % 600u == 0) {
      lucent::debug("skipmap",
                    "scanned {} fields: start_edges={} boot_fields={} stage_fields={} "
                    "current={}",
                    fields,
                    start_edges,
                    boot_fields,
  ...
  // reason, not a broken one: repl.read() is only pumped from the framework's native scheduler
  // loop, and that loop never runs here because the guest still owns its frame loop. Pumping it
  // from this frame boundary costs nothing and makes a live port inspectable instead of requiring a
  ...
  // An explicit guest/native field boundary must close the framework host turn even when FPS60
  // owns the sleep in frame_commit().  A clock-released hostturn field does neither: it is already
  // released and acknowledging it again would cancel the next timer interval.
  if (acknowledgeHostTurn) {
  ...
  for (uint32_t cls :
       {c->cfg->irqEventClasses[0], c->cfg->irqEventClasses[1], c->cfg->irqEventClasses[2]}) {
    if (cls) {
      c->game->hle.deliverEvent(cls, 0xFFFFFFFFu);
    }
  }
  ++g_pace_vbl;
  lucent::debug("pace",
                ...
                g_pace_present,
                g_pace_ack,
                g_pace_rq_unconsumed,
                site,
                c->cfg ? c->cfg->paceQuota : 0u,
                c->mem_r32(kVblankCounter),
                rq_n,
                rq_unconsumed);
  g_in_field = false;
  // THE PRODUCER DB'S FRAME BOUNDARY, and the only one this port has. Everything above this line is
  // here for the same reason (flush, present, pace, audio, events): the guest owns its frame loop,
  // so the framework's native_step_frame never runs and THIS field is the port's per-frame point.
  // The DB needs it for the one thing it could not otherwise have — an END.
  // `PSXPORT_NATIVE_FRAMES=<n>` makes this call end the run at frame n with the report written;
  // uncapped it only counts. See producer_run.h for why a periodic flush here would be wrong
  // (appendClaims does not dedup).
...
void vblank_wait(Core *c) {
  ...
  // A VSync WAIT reached from inside the vblank handler cannot be satisfied: the field it is
  // waiting for cannot begin until this one ends. That is a deadlock on the console too (the BIOS
  // runs the root handler with interrupts masked), so there is no behaviour to be faithful to — but
  // the port must not spin silently either. Say it, with the caller, and let the guest continue.
  if (g_in_field) {
    lucent::error("vsync",
                  "VSync wait for {} entered from INSIDE the vblank handler (ra=0x{:08X}) — "
                  ...
                  c->r[31]);
    if (cur < target) {
      c->mem_w32(kVblankCounter, (uint32_t)target);
    }
    return;
  }
  ...
  while (cur < target) {
    if (advanced >= kMaxVblanksPerWait) {
      lucent::warn("vsync",
                   "wait target {} is {} vblanks ahead of the counter ({}) — clamped at {}. "
    ...
    if (deliver_field(c, "vsync", true, true, true)) {
      cur = (int32_t)c->mem_r32(kVblankCounter);
    } else {
      cur++;
    }
    advanced++;
...
// depth" is the first question any corruption blamed on it has to answer, and it cannot be
// recovered after the fact. The counter is bumped unconditionally so a run can report how many
// turns it took even with the channel off (`turns=` on the pace line's site=hostturn entries is the
// same number).
...
void spyro_host_turn(Core *c) {
  ...
  lucent::debug("hostturn",
                "turn #{} at pc=0x{:08X} ra=0x{:08X} sp=0x{:08X} gp=0x{:08X}",
                g_host_turns,
                c->pc,
                c->r[31],
                c->r[29],
                c->r[28]);
  deliver_field(c, "hostturn", true, false, false);
}
...
void spyro_boot_skip_end() {
  lucent::debug("bootskip",
                "disarmed: scanned {} boot fields, saw {} fresh edge(s), advanced {} "
                "time(s)",
                g_boot_skip.fields,
                g_boot_skip.edges,
                g_boot_skip.advances);
...
bool spyro_deliver_field(Core *c, const char *site, bool fps60CommitPending) {
  // This flag is armed only by the native renderer immediately before its matching frame_commit.
  // Boot/reference VSyncs therefore retain the ordinary present+pace path even when FPS60 is on.
  return deliver_field(c, site, !fps60CommitPending, !fps60CommitPending, !fps60CommitPending);
}

void spyro_fps60_commit_field_delivered(Core *c) {
  // Restart the host-turn deadline only after BOTH delegated presents/paces complete. Restarting it
  // inside either native field lets the timer expire during frame_commit and inject a third field.
  rec_host_turn_field_delivered(c);
  ++g_pace_ack;
  lucent::debug("pace",
                "fps60 commit ack: vbl={} pace={} present={} ack={}",
                g_pace_vbl,
                g_pace_entries,
                g_pace_present,
                g_pace_ack);
...
void spyro_register_vsync(Game *g) {
  ...
  // and if a PAL game ever needed this the fix is for GpuState to re-arm on a standard change, not
  // a literal here.
