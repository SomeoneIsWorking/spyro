...
//
// SUPER-CALL, NOT REPLACEMENT. This handler logs and then calls the recompiled body, so behaviour
// is unchanged and the body stays live and diffable. That is the framework's override design and
// the right first step of owning a function: observe it faithfully before reimplementing any of it.
...
void cd_service(Core *c) {
  ...
  if (lucent::channel_on("cdq")) {
    st = c->mem_r32(kStatus);
    pend = c->mem_r32(kPending);
    q = c->mem_r32(kQueued);
    arg = c->mem_r32(kReqArg);
  ...
  lucent::debug("cdq",
                "service: gate={} status=0x{:X}->0x{:X} pending={}->{} queued={}->{} arg=0x{:08X}",
                gate,
                st,
                c->mem_r32(kStatus),
                pend,
...
//
// On hardware libcd invokes it from the CD interrupt. This runtime raises no guest interrupts, so
// it is never called and the gate is never cleared — which is the whole of the post-splash stall.
//
...
void cd_retry_step(Core *c) {
  ...
  if (n < 8) {
    lucent::debug("cdq",
                  "retry#{} ENTER gate=[0x80076BB8]={} a={} b={} status=0x{:X}",
                  n,
                  c->mem_r32(kGate),
                  (int)c->mem_r32(kA),
                  (int)c->mem_r32(kB),
  ...
  // per-sector copy from Cd::setloc_lba into it was tried. Predicted effect: the guest advances
  // past its first sector. Observed: it re-issued the SAME read at LBA 37 and nothing advanced —
  // frames stayed at 8. An inferred destination that does not produce its predicted behaviour is an
  ...
  //
  // Also learned: the edge-trigger below LATCHES. The guest re-issues its read (re-setting the
  // gate) before the next sample, so `delivered` never re-arms and exactly one completion is ever
  // sent. Whatever replaces this must key off the read ISSUE, not off observing the gate at 0.
  //
  ...
  // guest issues its next request), which is what proved the gate/callback analysis right. But it
  // is only half of a read: the guest is told "your read finished" when nothing landed in its
  // buffer. Observable consequence, and the reason this is not mistaken for working: the guest
  // re-seeks the SAME sector every time — PSXPORT_DEBUG=cd shows only "LBA 37", never an advancing
  // position — so it is retrying, not loading.
  //
  ...
  if (cd_completion_pending) {
    cd_completion_pending = false;
    deliver_cd_complete(c);
  }

  if (n <= 8) {
    lucent::debug("cdq",
                  "retry#{} EXIT  status=0x{:X} pending={} queued={}",
                  n - 1,
                  c->mem_r32(kStatus),
                  c->mem_r32(kPending),
                  c->mem_r32(kQueued));
...
#define CD_PROBE(NAME, ADDR)                                                                       \
  void probe_##NAME(Core *c) {                                                                     \
    ...
    if (hits < 4)                                                                                  \
      lucent::debug("cdq",                                                                         \
                    "probe " #NAME " (0x{:08X}) a0=0x{:08X} a1=0x{:08X} a2=0x{:08X}"               \
                    " a3=0x{:08X}",                                                                \
                    (unsigned)ADDR,                                                                \
                    c->r[4],                                                                       \
                    c->r[5],                                                                       \
...
// contract the reference consumer owns via cdFileLoad, and it is the right layer: the destination
...
void cd_loader(Core *c) {
  ...
  // an image's signature matches the RAM at its base only until the game mutates the image's header
...
// THE SECOND READ PATH, and the one that actually loads levels. 0x80016500 (11 static call sites)
// is only the SYNC boot loader; 0x80016698 has NINETEEN call sites spanning 0x80014608-0x80015BC0
// and is what the streaming machine 0x80015370 drives. Missing it is why "6 loader calls, none a
// level" read as "the game never asks for the level" when in fact it asks ~10 times through here.
//
// Same argument contract, confirmed from the body rather than assumed — its prologue moves
// a0->s2, a1->s4, a2->s3, a3->s1 and reads the 5th argument from sp+0x40, then at 0x800166F0
...
#define LOADER_PROBE(NAME, ADDR)                                                                   \
  void lp_##NAME(Core *c) {                                                                        \
    ...
    if (h < 3)                                                                                     \
      lucent::debug("cdq",                                                                         \
                    "LOADER? " #NAME " a0={:08X} a1={:08X} a2={:08X} a3={:08X}"                    \
                    " sp={:08X}",                                                                  \
                    c->r[4],                                                                       \
                    c->r[5],                                                                       \
                    c->r[6],                                                                       \
...
void lp_800127C0(Core *c) {
  ...
  gen_func_800127C0(c);
  lucent::debug("skipmap",
                "boot sequence EXIT call={} phase={} vblank={}",
                calls,
                c->mem_r32(0x80075864u),
                c->mem_r32(0x800749E0u));
  spyro_boot_skip_end();
...
void probe_8005CBB0(Core *c) {
  ...
  if (h < 6) {
    lucent::debug("cdq",
                  "stall#{} ENTER a0={} A[0x800730F0]={} B[0x80073588]={} arg[0x800730E8]=0x{:08X}",
                  h,
                  a0,
                  c->mem_r32(kPollA),
                  c->mem_r32(kPollB),
...
void probe_8005BB78(Core *c) {
  lucent::debug("cdq",
                "EVENT-OPEN site a0=0x{:08X} a1=0x{:08X} a2=0x{:08X} a3=0x{:08X}",
                c->r[4],
                c->r[5],
                c->r[6],
                c->r[7]);
  gen_func_8005BB78(c);
  lucent::debug(
      "cdq", "EVENT-OPEN -> handle=0x{:08X} stored=0x{:08X}", c->r[2], c->mem_r32(0x800730E8u));
}
...
void probe_8005DB84(Core *c) {
  static unsigned h = 0;
  if (h < 4) {
    lucent::debug("cdq", "EVENT-TEST a0=0x{:08X}", c->r[4]);
  }
  h++;
  gen_func_8005DB84(c);
  if (h <= 4) {
    lucent::debug("cdq", "EVENT-TEST -> v0={}", c->r[2]);
  }
}
