...
- PROGRESS 2026-08-06 (C167): parts (1) and (2) are DONE game-side, and the FIRST PRODUCER of part (3) has landed.
  ...
  * (5) THIRD PRODUCER, input/codec slice only: game/render/fx_paired_actor.cpp resolves all three 0x80023AC4 animation layers and their guest /16 pose blend into host-side RTPS inputs; game/render/paired_actor_decode.cpp owns a pure normal-stream primitive/material/OT decoder with synthetic negative tests. The generated-code audit corrected its triangle OT weight to `z0 + z0/2 + z1 + z1/2 + z2` and preserved the R3000 low-five-bit variable-shift mask; quads sum four depths. The exact-PC pre-GTE oracle proves the model bytes are cumulative boundaries 100/222/238, hence layer counts 100/122/16 = 238 vertices plus three pipeline warm-ups. After correcting layer-0's reversed byte/short pointers and the guest's arithmetic DR0 pack (`z*65536+y`, including negative-Y borrow), real unblended and paired-INTPL frames both report `target_rtps=241/241 compared=238/238 mismatches=0`. It emits zero faces and the normal handler still refuses state 2.
- NEXT, in the abort's own order: (a) add exact projection, normal face emission and RenderQueue ownership for 0x80023AC4; dynamically reach and verify its plane-clipping arm before calling the producer complete (resolved XYZ is now byte-exact); (b) stage 13 modes 1 and 2 of 0x8007CEE4 (page-driven text screens, 3-slot save screen) — RE'd but never reached by a drivable run; (c) the 3D backdrop shared by stage 13 and the FIELD: 0x800521C0, 0x8001F158, 0x8001F798, 0x800258F0, 0x8004EBA8.
