...
- notes: RenderWorldChunks is structurally mapped (C198) AND measured at its game-state input boundary (PSXPORT_WORLD_CENSUS, 2026-08-17): a reference-leg run of 5000 presented frames hit 2088 live calls — 1555 via the flat list (a0<0, the render-driver call; 169,495 sector entries, max 109/call) and 533 via occlusion groups (a0>=0; 72,879 byte-indexed entries, max 178/call) — and moved 40,684,452 pool bytes with ZERO empty calls. The two list types are distinct (count-based flat vs 0xFF-terminated byte-indexed occlusion groups) and must not be conflated in the oracle. Next: capture the SZ3 range + per-face OT bins from the scratchpad during a live call, then begin the oracle. Clip-bound immediates 0x10000/0x1000000/0x2000000 at 8 sites. RasterizeSpritePrimQueue 0x80022A2C is measured at its game-state input boundary across all four variants (C176).
