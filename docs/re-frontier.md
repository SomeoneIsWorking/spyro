...
- gap: NOT blocked — docs/issues/0013 resolved. psxport's overlay_router.cpp already models a shared arena: Tomba!2's stage overlays likewise all load to one base and the resident one is identified by CONTENT SIGNATURE, so 37 overlays at 0x8007AA38 need one SLOT, not a redesign. Wiring: (1) GameConfig overlaySlots[0] = {0x8007AA38,"ARENA"}, (2) call overlay_note_load(c,dest) from the native loader in cd_queue.cpp right after the copy — the signature matches then, before the game mutates the image header, (3) add overlays to ensure_recomp.py OVERLAYS sharing that base (overlay_bases is keyed by TAG, so several tags may name one address). What remains genuinely unknown is which WAD entry each overlay is and how the game selects it: the per-call selector is a3 (byte offset into the archive) plus a fifth STACK argument, not a0 (a0 is WAD.WAD's base LBA 37). Tools: tools/callsite_args.py (I006), tools/wad_index.py (I005).
