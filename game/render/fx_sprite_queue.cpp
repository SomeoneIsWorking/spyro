...
bool setup_screen_gte(Core *c, uint32_t actor) {
  ...
  if (angle) {
    ...
    gte_write_data(0, (uint32_t)(uint16_t)(-(int16_t)si));
    gte_write_data(1, co);
    gte_op(c, 0x4A486012u);
...
uint32_t flat_colour(Core *c, uint32_t actor_flags, uint32_t normal) {
  ...
  if (boost) {
    ...
    if (base > 0) {
      ...
      const auto ch = [add](uint32_t v) {
        return (uint32_t)std::min(255, ((int)v + add) >> 4);
      };
      colour = ch(gte_read_data(9)) | (ch(gte_read_data(10)) << 8) | (ch(gte_read_data(11)) << 16);
    }
  }
  return colour;
}
...
void append_pending_actors(Core *c) {
  ...
  if (qi < kQueueCapacity) {
    ...
    lucent::error("spriteq",
                  "native queue builder exhausted all {} entries; no terminator fits",
                  kQueueCapacity);
  }
}
...
bool emit_screen_queue(Core *c) {
  ...
  for (uint32_t qi = 0; qi < kQueueCapacity; ++qi) {
    ...
    if ((c->mem_r8(actor + 0x50u) & 0x80u) == 0) {
      rejected_world++;
      continue;
    }
    ...
    for (uint32_t pi = 0; pi < nprim; ++pi) {
      ...
      if ((packed & 1u) == 0u || (packed & 2u) != 0u) {
        rejected_variant++;
        continue;
      }
      uint32_t vi[4] = {(packed >> 23u) & 0x7Fu,
                        (packed >> 16u) & 0x7Fu,
                        (packed >> 9u) & 0x7Fu,
                        (packed >> 2u) & 0x7Fu};
      ...
      if (vi[0] >= nvtx || vi[1] >= nvtx || vi[2] >= nvtx || vi[3] >= nvtx) {
        continue;
      }
      ...
      // result and saturation behavior exact; the host substitute dropped ten live faces at timer
      // 171.
      for (int i = 0; i < 3; ++i) {
        const uint32_t sxy =
            (uint16_t)projected_x[vi[i]] | ((uint32_t)(uint16_t)projected_y[vi[i]] << 16);
      ...
      if (!visible && count == 4) {
        const uint32_t sxy3 =
            (uint16_t)projected_x[vi[3]] | ((uint32_t)(uint16_t)projected_y[vi[3]] << 16);
      ...
      for (int i = 0; i < 4; ++i) {
        rs[i] = (uint8_t)colour;
        gr[i] = (uint8_t)(colour >> 8);
        bs[i] = (uint8_t)(colour >> 16);
      }
      c->game->rq.emitOrQueue(c,
                              1,
                              RQ_HUD,
                              RQ_OM_2D_FG,
                              count,
  ...
  lucent::debug("spriteq",
                "native screen queue: present={} state={} timer={} emitted={} candidates=(tri "
...
void trace_reference_faces(Core *c) {
  ...
  for (uint32_t qi = 0; qi < kQueueCapacity; ++qi) {
    ...
    for (uint32_t pi = 0; pi < nprim; ++pi) {
      ...
      const int count = vi[2] == vi[3] ? 3 : 4;
      for (int i = 0; i < 3; ++i) {
        gte_write_data(12u + (uint32_t)i,
                       (uint16_t)px[vi[i]] | ((uint32_t)(uint16_t)py[vi[i]] << 16));
      }
      gte_op(c, 0x4B400006u);
      ...
      bool visible = maca > 0;
      if (!visible && count == 4) {
        gte_write_data(12, (uint16_t)px[vi[3]] | ((uint32_t)(uint16_t)py[vi[3]] << 16));
        gte_op(c, 0x4B400006u);
        macb = (int32_t)gte_read_data(24);
        visible = macb < 0;
      }
  ...
  lucent::info("spriteface",
               "leg=reference timer=171 scanned_records={} candidates={}",
               records,
               candidates);
  lucent::info("spritematrix",
               "phase=replica cr={:08X},{:08X},{:08X},{:08X},{:08X}",
               gte_read_ctrl(0),
               gte_read_ctrl(1),
               gte_read_ctrl(2),
               gte_read_ctrl(3),
...
bool SpyroRenderer::stage13Mode3Render() const {
  ...
  for (uint32_t a :
       {0x80076EF9u, 0x80076EFAu, 0x80076EFBu, 0x80076F7Du, 0x80076F7Eu, 0x80076F7Fu}) {
    c->mem_w8(a, 0);
  }
  c->mem_w32(kQueue, 0);
