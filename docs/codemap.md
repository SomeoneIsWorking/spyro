...
| Natively OWNED guest code | `game/core/native_rand.cpp` | 🟡 | **The honest ratio: 20 observation wrappers to 2 native bodies** (C075). Most overrides super-call the recompiled body; the CD/pad ones are platform-level *supply*. Genuinely owned: the vblank wait, and `rand()` `0x8006272C`. Every native body is gated byte-exact against the body it replaced on each gate run (`PSXPORT_NDIFF`, I019) — validated in both directions, and it caught a real `$at` inequivalence in the first replacement that reading the code did not. Next targets: frontier `own.next-targets`. |
