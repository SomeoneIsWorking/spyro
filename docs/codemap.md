...
| Independent behavior comparison | Drive the Lightrec product and the Beetle full-console reference to title-declared state checkpoints with identical pad delivery and compare declared main-RAM ranges | `tools/oracle_compare.py` (launch, report), `tools/oracle_spyro1.py` (predicates, declared/excluded state, game-frame barrier, input policy), `external/psxport/tools/oracle/compare.py` (shared driver) | `uv run --frozen python tools/oracle_compare.py [--selftest]` | `external/psxport/docs/oracle.md`, `docs/issues/0110-artisans-camera-checkpoints-are-not-yet-phase-aligned.md` |
