// wide_clip.cpp — widescreen, by moving the guest's own clip bounds instead of replacing its
// renderers.
//
// THE BLOCKER, AND WHY IT LOOKED LIKE A WALL. Every one of this game's hand-written assembly
// geometry renderers rejects faces against clip bounds that are IMMEDIATE constants in its own
// instruction stream: `lui rX, 0x0200` is sx >= 512, the 4:3 right edge. The static recompiler
// bakes those into C literals, so nothing downstream can move them, and a renderer whose bounds
// cannot move cannot draw the extra columns of a wider frame. From there the conclusion followed
// that widescreen needs every contributing renderer OWNED — reimplemented natively, byte-exact,
// ~9150 instructions of assembly across four functions (C133/C137) — and that re-centring the
// projection is all-or-nothing across them, so nothing is visible until the last one lands (C135).
//
...
// merely looked for `lui rX, 0x0200` was wrong once already (C133 — 0x800580F4's is a packet tag,
// not a bound), and six more tag sites inside 0x800258F0 alone would have been mis-patched by it. A
// site qualifies only when the surrounding code is the clip-code idiom — the 0x00010000 /
// 0x01000000 / 0x02000000 triple loaded together, then consumed by `sub` against the packed screen
// word to set bits 1/2/4/8. Anything else with the same immediate is a GPU command word.
//
...
// site a bound rather than a tag — cited so the next reader can check the classification rather
// than trust it.
struct Renderer {
  uint32_t addr;
...
// it cannot be moved the same way — which is fine, because the extra width is placed on both sides
// by re-centring the projection, not by moving the left bound out.
const Renderer kRenderers[] = {
    ...
    // normal run this renderer's bound and projection come from the recompiled body like every
    // other.
    // Leaving it out was visible immediately: the ground and the characters moved by the margin and
    // the SKY did not, which is exactly the self-misalignment C135 warns about, produced by
    // omission
    // rather than by design. spyro_register_wide_clip() skips this entry when the native body is
    // on.
    {0x8004EBA8u,
...
// not answer it: the frames differed by FIVE pixels, and a column-correlation search between the
// 4:3 and 16:9 captures reported its own SEARCH BOUND as the best match in every band — this scene
// is mostly flat sand and does not discriminate. A metric that cannot separate the two answers is
...
void emit_report(Core *c, int ri, int wide, uint32_t before, uint32_t after) {
  ...
  if (!lucent::channel_on("wideprims")) {
    return;
  }
  Emit &e = s_emit[ri][wide];
  e.calls++;
  if (after >= before) {
    e.bytes += (after - before);
  }
  if (after == before) {
    e.zero_calls++;
  }
  ...
  lucent::debug("wideprims",
                ...
                after,
                other.calls,
                other.calls ? other.bytes / other.calls : 0);
}
...
void run(Core *c, int ri) {
  ...
  if (!gpu_vk_wide_engine(c) || s_refused[ri]) {
    ...
    if (!s_said_43[ri]) {
      ...
      lucent::info("wide",
                   "0x{:08X} ({}) called, but NOT widened: wide_engine={} refused={} — "
                   "running the recompiled body unchanged.",
                   R.addr,
                   R.what,
                   gpu_vk_wide_engine(c),
                   (int)s_refused[ri]);
    }
  ...
  // rather than undoing what was installed around the call.
  if (!s_refused[ri] && R.ofx_site) {
    if (!s_have_ofx[ri]) {
      const uint32_t w = c->mem_r32(R.ofx_site);
      if ((w & 0xFFE0FFFFu) != 0x3C000100u) {
        lucent::error("wide",
                      "0x{:08X}: expected `lui rX,0x0100` at the OFX reset site 0x{:08X}, "
  ...
  if (s_refused[ri]) {
    for (int i = 0; i < n; i++) {
      if (s_have_orig[ri][i]) {
        c->mem_w32(R.sites[i], s_orig[ri][i]);
      }
    }
    run(c, ri);
    return;
  ...
  // Say it ONCE per renderer, with the before and after word. A widening that silently does not
  // fire is indistinguishable from one that fires and changes nothing — and this project has
  // recorded a "no effect" non-result that was really a measurement fault more than once (issue
  // 0039).
  if (!s_said[ri]) {
    s_said[ri] = true;
    lucent::info("wide",
                 "0x{:08X} ({}): {} bound site(s) widened 512 -> {}, first at 0x{:08X} "
  ...
  // its vertices are outside, and with the projection still centred on 256 almost nothing lies
  // wholly beyond 512. The two halves only work together: OFX at nw/2 pushes the world right by the
  // margin, which brings faces that were wholly off the LEFT edge into view (the left bound is a
  // compare against zero, so they stop being rejected on their own), and the widened right bound is
  // what keeps the faces that move PAST 512 from being rejected in exchange.
  //
  ...
  // others keep theirs slides its content off the geometry it belongs to, with visible seams
  // (C135).
  ...
  // Put the guest's own instructions back. A bound must not stay widened for any other reader of
  // this code — and leaving it patched would make a later 4:3 toggle silently wrong.
  for (int i = 0; i < n; i++) {
    c->mem_w32(R.sites[i], s_orig[ri][i]);
  }
  if (R.ofx_site) {
    c->mem_w32(R.ofx_site, s_ofx_orig[ri]);
  }
}
...
void spyro_register_wide_clip() {
  ...
  // bound — installing over it here would silently displace the very thing that bring-up is
  // testing.
  if (!cfg_on("PSXPORT_NATIVE_TERRAIN")) {
    psxport_recomp()->shard_set_override(kRenderers[0].addr, hook<0>);
  }
  ...
  lucent::info("wide",
               ...
               "right bound moved to the wide width.",
               kRendererCount);
}
