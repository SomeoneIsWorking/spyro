...
//
// The guest's render driver 0x8001ED5C is a linear if-chain over one stage selector, and each arm is
// a different screen (front-end, title, field, …). So "what am I being asked to draw" has a concrete
// answer in game state, and it is the answer a native producer has to dispatch on. This file holds
// that identity: the selector, the arms transcribed from the chain, and the FIELD arm's layer list.
//
// NOTHING HERE IS NAMED BEYOND ITS ADDRESS unless this repo already named it. A stage whose role has
// not been reverse-engineered gets "(role not RE'd)" — an invented name would read as knowledge, and
// the whole point of this table is to be the porting backlog rather than a story about one.
#include "render.h"
#include "core.h"
#include "guest_gp.h"
...
void SpyroRenderer::reportBacklog(const Scene& sc) const {
  if (!sc.arm) {
    lucent::error("render", "  stage {} is outside 0..15: the guest's render driver 0x{:08X} falls "
  ...
  if (sc.stage == 7)
    lucent::error("render", "  the guest would have called (*[0x{:08X}]) = 0x{:08X}",
                  kStageFnPtr7, mC->mem_r32(kStageFnPtr7));
  if (sc.stage == 13)
    lucent::error("render", "  [0x{:08X}] = {} selects 0x8001E6B8 (==3) or 0x8007CEE4",
                  kStageIndirectPtr13, mC->mem_r32(kStageIndirectPtr13));
  if (sc.stage == 15)
    lucent::error("render", "  [0x{:08X}] = {} selects 0x8007BFD0 (<99) or 0x8001EB80",
                  kStageIndirectPtr15, mC->mem_r32(kStageIndirectPtr15));
  if (sc.stage == 0) {
    lucent::error("render", "  the FIELD backlog, in the guest's own draw order — ARMED means the "
  ...
  lucent::error("render", "  projection (the game's own SetGeomOffset/SetGeomScreen values): "
                          "geomValid={} ofx={} ofy={} H={}",
