...
- NEXT, in the abort's own order: (a) find and drive the writer/scene that makes the 0x80023AC4 parser-control high byte nonzero, then oracle and own the separate alternate/status-plane clipping arm. The established 4100-present native corpus is a measured negative, not evidence of absence. Then (b) stage 13 modes 1 and 2 of 0x8007CEE4; (c) finish direct ownership of the 3D backdrop. For `0x8004EBA8`, the immutable-source capture gate is now real but ownership is not: generated-leg checkpoints independently matched native F3/G3 fields, packet chain and OT splice for 100 nonempty calls (`scratch/logs/terrain_recipe_oracle.log`), while pool exhaustion remains unseen. The next terrain step is an atomic direct RenderQueue producer and guest-packet suppression after complete preflight—not another packet-side shortcut. Other backdrop functions remain `0x800521C0`, `0x8001F158`, `0x8001F798`, `0x800258F0`.
