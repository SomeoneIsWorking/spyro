...
- gap: Native product rendering owns the measured display tail, one temporal commit, issue 0086's mode-2 save picker, and the reached stage 14 / `GS_Cutscene` route grounded in retained arm `0x8001E9C8`. The isolated real-disc C228 run verifies its shared actor/world/cyclorama owners, cutscene clear colour and `0x14000` world distance, and fade producer `0x800190D4` in a coherent 684-wide picture under the host frame loop; issue 0088 is resolved. The retained reference/GTE leg still dispatches 0x8001ED5C, whose VSync call hits the mandatory trap; split that diagnostic renderer tail without restoring a VSync success path. Native FIELD scene ownership remains a separate attract/gameplay-route gap.
