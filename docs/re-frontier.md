...
- PROGRESS 2026-08-06 (C167): parts (1) and (2) are DONE game-side, and the FIRST PRODUCER of part (3) has landed.
  ...
  * (5) THIRD PRODUCER, joined normal opaque/textured path: game/render/fx_paired_actor.cpp resolves all three 0x80023AC4 animation layers, builds CR0..7 and the exact CR13..15 depth controls from current instance/model/camera state, uses owned CR24..26, projects and resolves the normal stream, then submits exactly one object-local authored-order group through framework `2ce8e26c`'s painter compositor. No ambient GTE state or guest-compute fallback remains. Across 384 reference invocations the gate matched 3/3 layer snapshots, 36/36 CR values, 6/6 root inputs, 238/238 vertices, and ordered packet content. A separate 4100-present float gate compared the game-owned projector to framework `proj_native_xform`: 384/384 invocations × 238 vertices matched IR1..3, SZ, SXY, float XY and PZ with zero field mismatches. Rounded float XY remained within one pixel of guest SXY (maximum 1), and every vertex retained subpixel XY distinct from the integer-forced negative. The shipping group now supplies those pure production `screen_x/screen_y` values through `RqItem::xsf/ysf`. In paired 4100-frame runs the reference leg reported 2021 native-group 0/0 gates; the native leg accepted 274/274 armed groups with no refusal, and the first state-specific census matched at 355 candidates / 184 faces. GPU offset/window/clip state is captured once per invocation and TPAGE/CLUT per face. The measured 19 D32/order contradictions are handled by authored-order composition, not flattened. **Still open:** untouched global slots outside the splice window, alternate/status-plane clipping, and temporal/FPS60 endpoint ownership; semi, dither, unsupported texture mode, incomplete data, and a second invocation refuse.
- NEXT, in the abort's own order: (a) dynamically reach and own the separate alternate/status-plane clipping arm. The normal path now has immutable host-only pose/transform/primitive/material/GPU capture and an explicit-queue pure endpoint rebuild that reprojects and reruns NCLIP; next prove its previous/current rotation against the framework's two-present lifecycle, then interpolate captured inputs without reading guest selectors at present time and wire the result into the FPS60 world pass. Current draw-frame rotation is scaffolding, not a certified present-time contract. (b) stage 13 modes 1 and 2 of 0x8007CEE4 (page-driven text screens, 3-slot save screen) — RE'd but never reached by a drivable run; (c) the 3D backdrop shared by stage 13 and the FIELD: 0x800521C0, 0x8001F158, 0x8001F798, 0x800258F0, 0x8004EBA8.
