...
`type2=2`, and stopped at the next genuine refusal, the actor pass. No particle refusal occurred.

## Proper next step

The type-2 recipe/submitter now owns the decoded packet family. Keep the source packet and table
facts above as the boundary contract; additional particle types remain separate work.

## Exit condition

The type-2 producer is source-grounded and admitted by the normal native FIELD composition, with
focused positive/malformed recipe tests and a live gate-route run that reaches the next genuine
refusal.
