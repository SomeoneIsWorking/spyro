...
//    START is reprogrammed here, and this file says so rather than implying it ported all of
//    libgpu.
//
// THE STRUCT LAYOUT IS NOT ASSUMED — IT IS CONFIRMED. Sony's DRAWENV puts (r0,g0,b0) at
// +0x19/1A/1B, and the guest's own stage-13 handler writes exactly 0x80076EF9/EFA/EFB and
// 0x80076F7D/7E/7F (scratch/decomp/title_stage13.c). Those are kEnvA+0x19.. and kEnvB+0x19.., which
...
uint32_t nativeFrameBegin(Core *c) {
  ...
  // can verify. If a future env breaks that alignment this warns instead of drawing the wrong
  // thing.
  if (de.isbg()) {
...
// env 0x80076F64 draws at y=240 and displays y=0). So "display what this env says" shows the frame
...
void nativeFrameEnd(Core *c, uint32_t env, bool fps60CommitPending) {
  ...
  while (now - (int32_t)c->mem_r32(kStampLastFrame) < kMinFieldsPerFrame) {
    if (fields >= kMaxFieldsPerFrame) {
      ...
      lucent::warn("frameenv",
                   "field throttle gave up after {} fields: counter={} stamp={} — the "
