...
| Temporal scene integration | Per-game source admission scratch, dynamic producer participation, camera/frame provenance, world resource residency history and shared-presenter reconstruction/rotation | `game/render/temporal_scene.*`, `world_temporal.*`, `actor_temporal.*`, `secondary_actor_temporal.*`, `scene_camera_inputs.h` | `spyro_temporal_scene_begin`, `spyro_temporal_scene_prepare`, `spyro_temporal_scene_source`, `world_temporal::History`, `actor_temporal::History`, `secondary_actor_temporal::History` | `docs/project-state.md` |
...
| Actor pairing and sampling | Matching one frame's actor records to the frame before them by Moby instance in occurrence order, the measured identity rule that decides whether two poses may be blended, and the census that names the rejecting field | `game/render/actor_pairing.*` | `actor_pairing::sample`, `actor_pairing::mismatch`, `actor_pairing::Census` | `docs/codemap.md` |
...
| Per-face actor colour | The two colour programs `func_80020F34` selects with prefix bit 2: the ported directional term over a triangle's view-space normal, and the named refusals for the additive arm and the quad billboard | `game/render/face_light_program.*` computes it purely; `face_light_environment.*` snapshots the guest light colour and magnitude table per composition; applied in `actor_draw_recipe.cpp` | `face_light::face_color`, `face_light::EnvironmentSource` | `docs/issues/0113-attract-demo-aborts-secondary-shaded-producers-r.md` |
...
| Independent behavior comparison | Drive the Lightrec product and the Beetle full-console reference to title-declared state checkpoints with identical pad delivery and compare declared main-RAM ranges | `tools/oracle_compare.py` (launch, report), `tools/oracle_spyro1.py` (predicates, declared/excluded state, game-frame barrier, input policy), `external/psxport/tools/oracle/compare.py` (shared driver) | `uv run --frozen python tools/oracle_compare.py --frame-step 1 [--selftest]` | `external/psxport/docs/oracle.md`, `docs/issues/0110-artisans-camera-checkpoints-are-not-yet-phase-aligned.md` |
