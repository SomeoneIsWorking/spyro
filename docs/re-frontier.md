...
- where: game/core/frame_loop.cpp (UNCONDITIONAL since 2026-08-17 — the port owns the loop on every run; PSXPORT_SPYRO_FRAME_LOOP retired); game/core/game_hooks.cpp spyro_bootInit; the render seam it calls is game/render/render_frame.cpp
- gap: OWNED: the outer loop — and it is now THE path, unconditional (2026-08-17: the env gate was retired because reaching a drawn frame must not require a flag; the port boots, reaches the title, and its native producers draw on a plain run). NOT owned: present/pace (still vsync.cpp, one per VBLANK — moving it into the loop 1:1 would HALVE the present rate, because Spyro's logic frame is ~30Hz and the display 60Hz) and the render driver 0x8001ED5C, which is the actual native-graphics seam. THE PREVIOUS TEXT OF THIS ENTRY WAS WRONG ABOUT THE DIRECTION and is corrected here: it said the blocking item was generalising psxport's per-frame GameConfig group to a per-parity-selector model. That framework change is NOT needed and must not be attempted for this reason — the group only feeds native_step_frame, which is UNREACHABLE in this port. native_step_frame is reached only from game_main (via native_crt0 <- native_boot_run <- BootStub::run) and from dc_step_frame; BootStub::run has ZERO callers in spyro (Spyro boots one executable, no SCEA stub) and dc_boot_init never returns here, so even PSXPORT_SELFTEST cannot reach it (measured: PSXPORT_SELFTEST=startgame hangs with a stack of dc_boot_init -> gen_func_80012204 -> gen_func_8001ED5C). Tomba!2 IS the port that calls BootStub::run, which is why its native renderer hangs off native_step_frame. C073 stays correct and its conclusion (leave those GameConfig fields 0) is now permanent, not provisional. ALSO FIXED 2026-08-17: the render seam now calls fps60.frame_commit unconditionally (the framework's flush captures in both configs and frame_commit is the only drain — gating it on fps60.active() overflow-aborted the default fps60-off run), and the reference leg drains via the new Fps60::reset_capture.
...
- PROGRESS 2026-08-06 (C167): parts (1) and (2) are DONE game-side, and the FIRST PRODUCER of part (3) has landed.
  ...
  * (4) SECOND PRODUCER, verified semantic layer only: game/render/fx_sprite_queue.cpp owns 0x8001E6B8's text construction plus the live screen-space/flat-bit01 class of 0x80022A2C. At stage-13 timer 171 it matches the guest's isolated output: 551 candidates, 232 accepted = 25 triangles + 207 quads. C177's earlier pixel claim remains falsified because those captures exercised the prior title producer. State 2 now joins the separately owned three-layer 0x80023AC4 producer described below.
  * (5) THIRD PRODUCER, joined normal opaque/textured path: game/render/fx_paired_actor.cpp owns all three 0x80023AC4 animation layers, transforms, exact endpoints, and the continuous interior FPS60 policy. Host-only recipes rotate after both presents through framework `bf0a2b01`; present-time code reads no guest state. Because live animation changed integer NCLIP/split/bin output on all 160 otherwise-compatible intervals, interior passes resolve the stable source stream with interpolated raw view XYZ, float signed-area visibility, and a continuous authored OT key; exact t=0/1 retain the integer guest resolver. Paced and audio-enabled 4100-present runs each produced 320 temporal passes (160 midpoint + 160 endpoint), no refusal/FATAL, and `wrong_half=0/1830`; forced endpoint runs likewise completed 320 passes each. The measured 19 internal D32/order contradictions remain handled by PainterObject composition. **Still open:** alternate/status-plane clipping, semi/dither/unsupported modes, untouched splice slots, and all other world producers.
- NEXT, in the abort's own order: (a) find and drive the writer/scene that makes the 0x80023AC4 parser-control high byte nonzero, then oracle and own the separate alternate/status-plane clipping arm. The established 4100-present native corpus is a measured negative, not evidence of absence. Then (b) stage 13 modes 1 and 2 of 0x8007CEE4; (c) continue direct ownership of the 3D backdrop. `0x8004EBA8` now has an atomic direct F3/G3 PainterObject producer: its generated-leg oracle matched 8,368 faces and all 11,800 guest N+1 vertex iterations across 17 calls, including source order, projection/content and a forced-corruption negative; a fresh 4100-present native run emitted 876,293 faces across 1,557 nonempty frames with no guest packet/OT writes, no refusal/FATAL and `unscoped-native 0`, while 275 valid-empty calls remained empty. Terrain and paired identities are distinct, but the live corpus reached them on adjacent frame intervals; same-frame multi-object composition remains framework-test evidence, not a Spyro corpus observation. Pool exhaustion remains unseen. The remaining backdrop functions are `0x800521C0`, `0x8001F158`, `0x8001F798`, and `0x800258F0`.
  - FIELD BACKLOG MEASURED AND CORRECTED 2026-08-19 (C205, `tools/field_layers.py`). Stage 0's ten layers now ALL have RE'd roles, and the 3D subset is FIVE, not ten: 0x80019698 actor pass (1244 COP2 over its closure — mobys, shadows, Spyro, flame, glows, sparkles; reaches 0x8001F158/0x8001F798/0x80020F34/0x80022A2C/0x80023AC4), 0x8002B9CC environment (276, ALL of it inside RenderWorldChunks 0x800258F0 — the layer itself only picks the occlusion group and the culling distance, then calls the world renderer once), 0x80050BD0 cyclorama/sky (230, bottoms out in EmitStaticActorMeshList 0x8004EBA8), 0x800573C8 particles (166, ALL IN ITS OWN 843-instruction BODY), 0x800189F0 tracers (15, and those are the shared world->screen helper 0x80017B48 and isqrt 0x80017A38, not a renderer of its own). The other five are 0 COP2 over 64..459 instructions scanned each: 0x800521C0 moby list build, 0x80019300 collectables, 0x80018908 demo text, 0x800190D4 fade, 0x80018F30 border. All ten roles are corroborated by the vendored decomp's stage-0 arm (external/spyro-1/src/gamestates/draw.c:2716-2747), which has the same ten calls in the same order under the same gates — including the shift this repo had already derived on its own, [0x80075918]<<3 == g_Fade*8.
  - TWO CORRECTIONS THIS FORCES ON THE TEXT ABOVE. (i) 0x800521C0 is NOT a backdrop renderer: 64 instructions and no GTE traffic at all. It queues the level's mobys for the passes below, so the actor-chain ownership unit "0x800521C0 -> 0x8001F158 -> 0x8001F798" is a list build feeding two renderers, not three renderers. (ii) PARTICLES 0x800573C8 was missing from every backlog in this file, and it was missing for a STRUCTURAL reason worth keeping: it projects and emits inline and calls nothing, so walking the call graph to a known renderer scores it 0. That is why field_layers.py counts each layer's OWN body separately from its closure — the closure walk alone would have dropped a whole 3D layer from the backlog and nothing would have contradicted it.
  - SO THE FIELD'S REMAINING PRODUCER WORK IS: the actor pass (partly covered — 0x80022A2C and 0x80023AC4 have producers; 0x8001F798 and 0x80020F34 do not), the WORLD 0x800258F0 (body owned byte-exact, NO producer — the ground and the cliffs, and the depth source widescreen re-centering and 60fps interpolation both ride on), the cyclorama 0x80050BD0 (0x8004EBA8 has a direct producer; the layer's other COP2 sites 0x80016D2C/0x80050240/0x8004F4BC do not), PARTICLES 0x800573C8 (nothing), TRACERS 0x800189F0 (nothing), plus four small 2D layers.
...
- gap: 
- notes: PHASE DONE: the high-caller LEAF work is complete. tools/own_candidates.py now derives 25 owned bodies from the live ndiff_run sites; C081's historical count of 15 is superseded. Four are GTE bodies, owned without reimplementing the GTE — scalar logic native, COP2 via the platform's gte_op/gte_read_data/gte_write_ctrl, integer divide via cpu_div (C++ division is UB exactly where MIPS defines behaviour: /0 and INT_MIN/-1). Its best remaining LEAF has 15 callers, down from 136, so this seam has given what it has to give. CAVEAT on exhausted: the caller count is STATIC, so indirect calls are invisible and a low count is not proof of coldness — a runtime call histogram is the honest measurement. Continuing is tracked by own.non-leaf.

### own.non-leaf — Own NON-LEAF functions, bottom-up from the leaves already owned
- status: re-partial
- deps: own.next-targets
- evidence: C082,C207,C209,C210,C211,I019,I020,I028
- where: game/core/native_printf.cpp; game/core/native_actor_mesh_scratch.cpp; game/core/native_spu_pio_upload.cpp; game/core/spu_pio_upload.h; game/core/native_spu_hardware_init.cpp; game/core/spu_hardware_init.h; tools/own_candidates.py --ready-nonleaf
- gap: FOURTH STEP VERIFIED, not the family. PsyQ InitSpuHardware 0x8005BBF4 is now owned bottom-up: the 165-instruction SCUS_942.28 body calls only the already-owned spin60, printf, and WriteSpuRamPio boundaries. A mixed FNTRACE run reached it once at frame zero while the two higher static candidates were explicitly cold; the real call then matched the retained generated body across NDIFF's compared state. The focused test covers both reset-policy answers, the 24-voice tuple, and both loop bounds. Issue 0071 records why static rank alone cannot select the next body. Continue with the same dependency-ready queue, but require a known-positive FNTRACE batch before implementation.
- notes: C082 puts all guest code at only 4.5-4.9% of host CPU, so further ownership buys correctness and architecture, not speed. Pick from tools/prof_hot.py for measured speed work and from the static queue for coverage. FNTRACE and an owned override cannot be armed on the same address because both use its one override slot; C207, C209, C210, and C211 therefore cite separate reach and equality runs. NDIFF does not snapshot host-side Beetle SPU state; C211 records that blind surface rather than laundering the exact compared-state result into a whole-device equivalence claim.
...
- evidence: C127,C129,C130,C136,C137,C138,C139,C140,C141,C176,C177,C198,C199,C201,C203
- where: 0x8004EBA8 (cyclorama, owned byte-exact) · 0x800258F0 RenderWorldChunks (C198: full structural map — 0x800258F0..0x8002A6F4, ~5000 insns, 3 phases: cull+animate / world emit / special-surface emit) · + 17 more sharing the fixed-area register-save idiom at D_80077DD8
- gap: OWNERSHIP IS BACK ON WIDESCREEN'S CRITICAL PATH, but for a different and much better-defined reason than before. It is NO LONGER needed for clip bounds or OFX: those are handled by running the five contributing renderers interpreted with their bound immediates patched in guest RAM, which is bit-identical to the recompiled body (C139) and measurably recovers 9-18% more geometry per call at 16:9 (C142). What IS blocked is everything downstream of 2D-vs-3D DISCRIMINATION — psxport's 2D widen shifts the WHOLE FRAME a second time at this port's ~2.5% depth coverage (C143), and the uncovered-margin strip (issue 0039) is in the same class. That discrimination rides on per-primitive DEPTH, and depth is exactly what an OWNED body can emit and an interpreted one cannot. So own these for DEPTH; pick the target by which renderer's vertices are most needed. Per the corrected mute map (C147, confirmed by the vendored decomp's call sites: func_800258F0 is invoked with the occlusion group in src/environment.c, func_80020F34 renders specular mobys in src/gamestates/draw.c), the GROUND is 0x800258F0 RenderWorldChunks — NOT 0x80020F34 as this entry previously said; 0x80020F34 is EmitSecondaryActorPrimitives (a second character). Start with 0x800258F0.
- notes: 0x800258F0 RenderWorldChunks IS NOW OWNED, byte-exact in source and in execution (C203). game/core/world_body.inc is EMITTED by tools/transcribe.py (I050) as a bijective rendering of generated/shard_3.c gen_func_800258F0 — 5065 statements, 50 lui/addiu address pairs folded to named constants — and the gate re-derives the generated source from it exactly, so substrate regeneration cannot silently orphan it. Executed under PSXPORT_NATIVE_WORLD=1 PSXPORT_NDIFF=100000 on an 8000-frame reference-leg run: 1682 calls, ZERO divergences over RAM + scratchpad + all 31 GPRs + the COP2 file, with BOTH entry shapes covered (1555 flat-list a0<0, 127 occlusion-group a0>=0, kept as separate ndiff sites so a count cannot pass for coverage). HAND TRANSCRIPTION IS RULED OUT AT THIS SIZE and that is the reusable lesson: the attempt carried four defects in its first 40 of 5000 instructions (a delay-slot load made conditional, an unconditional increment made conditional, a register substituted, every rec_irq_poll dropped), none visible until all 5000 were typed because the differential only runs whole bodies. Use transcribe.py for the remaining 17 renderers sharing the D_80077DD8 idiom. NEXT: emit DEPTH. The body is a real C function with the projection sites addressable, so a depth hook is an annotation keyed to a guest address (transcribe.py --ann), injected at emit time and stripped before the round-trip compares, with its count printed so it can never mask a divergence. The SZ3 write sites are Phase 2 (0x800261A0..0x8002A0B0), scratchpad 0x1F800000 8-byte stride; C201 has the expected range (4..12806) to check the hook against. Still off by default (PSXPORT_NATIVE_WORLD): 1682 of the census's 3587 calls were verified, so the pool-exhausted and Phase-3 special-surface arms may not have fired.
