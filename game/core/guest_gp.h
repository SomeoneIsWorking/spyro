...
// records the same value from the same instruction pair, so this is not a second source of truth for
// it — it is that value made reachable from code that has no GameConfig in scope.
//
