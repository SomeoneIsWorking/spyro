...
- PROGRESS 2026-08-06 (C167): parts (1) and (2) are DONE game-side, and the FIRST PRODUCER of part (3) has landed.
  * (1) the env flip + draw-env programming: game/render/frame_env.cpp `nativeFrameBegin` — the flip verbatim from 0x8001ED5C's head, then GP0 E3/E4/E5/E1/E2/E6 and the isbg fill built from the game's own DRAWENV (layout CONFIRMED by the guest's own background-colour stores at env+0x19/1A/1B). The OT/packet-pool pointers are deliberately NOT written: the native leg emits no guest packets.
...
- NEXT, in the abort's own order: (a) stage 13 with [0x80078D78]==3 -> handler 0x8001E6B8 (in MAIN) — this is what the run aborts on TODAY; (b) stage 13 modes 1 and 2 of 0x8007CEE4 (page-driven text screens, 3-slot save screen) — RE'd in scratch/decomp/title_stage13.c but NEVER REACHED by any run this port can drive, so deliberately not transcribed; (c) the 3D backdrop shared by stage 13 and the FIELD: 0x800521C0, 0x8001F158, 0x8001F798 (EmitActorDrawList), 0x800258F0 (RenderWorldChunks), 0x8004EBA8 (EmitStaticActorMeshList).
...
### render.native-camera — A Spyro camera reader for native producers — Fps60::sceneCam is Tomba!2-shaped and would lie
- status: todo
- deps: frame.own-render-driver
- evidence: C158
- where: external/psxport/runtime/recomp/fps60.cpp:86-92 (the defect); a Spyro reader would live game-side
- gap: FRAMEWORK DEFECT, confirmed by reading: Fps60::sceneCam — described in its own comment as 'the ONE camera reader for the whole native projection path' — hardcodes Tomba!2's SCRATCHPAD LAYOUT in game-agnostic framework code: 'const uint32_t SCR = 0x1F800000; w0 = mem_r32(SCR+0xF8) ... T = mem_r32(SCR+0x10C/0x110/0x114)'. Nothing about that layout is generic. It is INERT in spyro today (zero callers of sceneCam in spyro/game, and RenderQueue::drawWorldQuad — the only setter of has_xyf — has zero callers here), so it is not currently producing a wrong picture; but the first native producer that reaches for the framework's camera would silently read Spyro's own scratchpad, which native_terrain.cpp already uses as a per-object VERTEX CACHE. The projection CONSTANTS are fine — ProjParams is fed by the game's own SetGeomOffset 0x80062618 / SetGeomScreen 0x80062638 (game_config.cpp), and requireGeom aborts rather than inventing them. It is only the view MATRIX that is hardcoded. Fixing it in psxport needs a coord claim; not needed until a native producer exists, but it must not be discovered by a wrong picture.
- notes: Do not 'solve' this by reading GTE control registers or a guest pre-composed matrix — that is the banned tap (PROTOCOL.md). A Spyro camera reader must come from the game's own camera state.
