...
| Spyro 1 handoff store observation | Opt-in read-only pre/post RAM and tick snapshots at four authenticated translated SW PCs, with per-target hit and JIT denominators | `titles/spyro1/core/handoff_store_observer.*`; composed by `spyro1_frame_driver.*` over the psxport per-Core Lightrec observer | `HandoffStoreObserver::arm`, `finish`, `report` | `docs/issues/0110-artisans-camera-checkpoints-are-not-yet-phase-aligned.md` |
