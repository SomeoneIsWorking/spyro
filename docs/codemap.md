...
| Stage-update return observation | Startup-configured, read-only Spyro 1 camera and player samples after the outer guest update returns | `titles/spyro1/core/stage_update_observer.*`; composed by `spyro1_frame_driver.*` | `StageUpdateObserver::afterReturn` | `docs/issues/0110-artisans-camera-checkpoints-are-not-yet-phase-aligned.md` |
