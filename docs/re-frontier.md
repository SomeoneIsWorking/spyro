...
- status: todo
- deps: cd.chokepoints
- evidence: C068,C069,C073,C150,C151
- where: game/core/game_config.cpp per-frame group
- gap: THE OT/PACKET-POOL GROUP IS NOW RE'd — this gap's previous text listed it as missing and was stale. Display init (C068): 512x240 double-buffered, set up at 0x800122D0-0x8001233C via SetDefDrawEnv 0x8005EA94 / SetDefDispEnv 0x8005EB4C, envs at 0x80076EE0 (env0) and 0x80076F64 (env1). The per-frame driver is 0x8001ED5C and it does the whole reset ABOVE its stage switch, so unconditionally: pick the OTHER env by comparing [0x80075888]; reload [0x800757B0] (packet-pool bump pointer), [0x80075820] and [0x8007581C] from that env's +0x70/+0x74/+0x78; derive [0x800756FC]=[0x80075710]=[0x80075780] as pool+0x1C000; zero [0x800758B0]; commit [0x80075888]. The frame ends PutDispEnv(env+0x5C) / PutDrawEnv(env) / DrawOTag(head) with DrawOTag=0x8005FD64, PutDrawEnv=0x8005FDD8, PutDispEnv=0x80060030 — all three identified from the game's OWN libgpu debug strings at 0x800118FC/0x80011910/0x80011944, not inferred. Head comes from 0x80016784(0x800), the game's OT collapse/terminate helper. Confirmed on a GAMEPLAY snapshot (snap_15000): stage [0x800757D8]=0, env0 current, OT ptrs = env+0x70 + 0x1C000 exactly. WHAT REMAINS is not RE but DESIGN: C073 (fresh) says Spyro's shape does not fit psxport's per-frame GameConfig model — one otBasePtr rewritten per frame — because Spyro keeps per-parity pointers INSIDE the draw envs and selects by pointer. So the native loop needs the framework's per-frame group generalised to a per-parity-selector model, which is a psxport change and must be claimed under the coord protocol. Also still true: C069 — GPU registers are reached through an initialised libgpu register block at 0x80074B34, so an immediate-address scan reports zero GPU accesses; do not read that as 'the game does not touch the GPU' (the C064/SIO mistake).
- notes: Do NOT start by writing a native loop. The blocking item is the framework's per-frame model (C073), not any remaining Spyro RE. Decompiled bodies: scratch/decomp/frameloop.c, scratch/decomp/libgpu_drawotag.c (Ghidra project spyro470 over scratch/raw/snap_470.bin).
