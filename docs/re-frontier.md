...
- NEXT, in the abort's own order: (a) find and drive the writer/scene that makes the 0x80023AC4 parser-control high byte nonzero, then oracle and own the separate alternate/status-plane clipping arm. The established 4100-present native corpus is a measured negative, not evidence of absence: `scratch/logs/paired_alternate_reachability.log` scanned 275 invocations with normal=275, alternate=0; generated 0x80024B60..24C30 only proves the branch begins with a distinct INTPL/status prepass. Do not implement it from synthetic state without a live packet oracle. Then (b) stage 13 modes 1 and 2 of 0x8007CEE4 (page-driven text screens, 3-slot save screen); (c) the 3D backdrop shared by stage 13 and the FIELD: 0x800521C0, 0x8001F158, 0x8001F798, 0x800258F0, 0x8004EBA8. Paired-actor temporal ownership is complete for its reached normal arm, not a substitute for those producers.
