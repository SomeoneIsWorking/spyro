// Spyro's picture owner. Native scenes compose producers and submit one render queue; the explicit
// reference entry executes the retail driver through the runtime executor. Its display tail still
// reaches the mandatory VSync refusal, so it is not a qualified reference-picture path.
// FrameDriver and FieldScheduler own logic, field delivery, input, presentation, and pacing.
