...
⬜ card.hwcard-events              Answer the memory-card probe Spyro's title screen waits on
      gap: FULLY ROOT-CAUSED, not started. The title screen cannot leave sub-state 1 because it waits on a memory-card I/O-end event that is never delivered: OpenEvent(0xF4000001 HwCARD, spec 4, mode 0x1000 EvMdINTR, handler 0x80067DD0) at 0x80067EA0, and 0x80067DD0 is never called (C119 has the full measured chain from the exit gate down). TWO pieces are missing (C120). (a) Hle::deliverEvent only marks slots fired and never invokes the handler; that is right for polled EvMdNOINTR events but wrong for EvMdINTR, and the framework already stores mode and func per slot, so this is a missing call rather than missing state. (b) Nothing delivers HwCARD for this game: psxport's Memcard model replaces the BIOS libcard B0-vector primitives, but Spyro drives the card through its OWN driver at 0x80066000-0x80069000, so that model never intercepts it. DECIDE FIRST what a PC port should answer — modelling the card fully, or reporting 'no card present' as real hardware does for an empty slot, which is a legitimate device state rather than a fake completion. What is ruled out is forcing 0x80069030's return or poking [0x80075B58]: that fakes a device transaction the game is entitled to observe and would advance the title while every later card operation misbehaves.
