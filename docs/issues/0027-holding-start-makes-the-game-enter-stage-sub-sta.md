...
### Note (2026-07-29)
DELIVERY GAP FIXED — AND IT IS NOT ENOUGH. The port now runs the guest's VBlank ROOT handler (0x8005E560, installed by libetc at 0x80073928[0], enabling I_MASK bit 0) instead of the single captured VSyncCallback slot. Measured: 0x8005E560 270841 calls, and slot 7's handler 0x80067CD4 270006 calls first at frame 835 — it was NEVER CALLED before. Gate 14/14, 41282 frames, 0 divergences, so delivering the whole chain every frame is safe. That closes the 'port special-cases exactly one callback' defect (C118) for every slot, present and future, rather than for this one instance.
