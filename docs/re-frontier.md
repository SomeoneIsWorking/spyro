...
- PROGRESS 2026-08-06 (C167): parts (1) and (2) are DONE game-side, and the FIRST PRODUCER of part (3) has landed.
  ...
  * (5) THIRD PRODUCER, input/codec/production-transform/projection/normal-face slice: game/render/fx_paired_actor.cpp resolves all three 0x80023AC4 animation layers and their guest /16 pose blend. Its production-owned builder reconstructs three layer CR0..7 tuples from current instance/model/camera inputs using pure fixed-point operations, with no ambient GTE state or guest-compute fallback, then uses the owned boot `ProjParams` CR24..26 for the reached front-end path to project all 238 vertices. Dynamic inline per-view OFX/OFY remains outside this claim. Across 384 live invocations the transform gate matched 3/3 layer snapshots, 33/33 CR values, 6/6 root RTPS inputs, and 238/238 projected vertices; CR0 and H perturbation controls both changed outputs. The final one-unit defect was full-word packing borrow in `(-rootY) + ((-rootZ)<<16)`, now protected by a hermetic negative-low test. game/render/paired_actor_decode.cpp owns the pure normal-stream decoder and acceptance/content resolver. All 355 source candidates close, and emitted packet content matches source/fragment, class, XY, RGB, attributes, opcode and semitransparency. Owner-qualified checkpoints at 0x800257A0/0x800258B0 scan all 288 local bins and simulate the exact global splice: all 69,881 packets match numeric bin/order and all 22,709 touched global pair/tag words match post-splice, with zero topology, clearing, global-word, or simulation errors; corrupt local topology, bin, global link, and same-comparator expected-word controls reject. Framework `2ce8e26c` now owns the required object-local authored-order/real-depth compositor. This replaced the accept-all resolver, speculative GTE replay, guest-captured transform dependency, and invalid late-snapshot instruments. **Still open:** join the native stages and submit through the compositor, untouched global slots outside the splice window, and the separate alternate/status-plane clipping arm.
- NEXT, in the abort's own order: (a) implement a non-flattening object-local authored-order/depth composition path for 0x80023AC4, then submit normal faces; the live discriminator found 19 opaque D32/OT contradictions among 207 overlapping pairs, so insertion-only submission is falsified. Dynamically reach and verify the separate alternate/status-plane clipping arm before calling the producer complete. (b) stage 13 modes 1 and 2 of 0x8007CEE4 (page-driven text screens, 3-slot save screen) — RE'd but never reached by a drivable run; (c) the 3D backdrop shared by stage 13 and the FIELD: 0x800521C0, 0x8001F158, 0x8001F798, 0x800258F0, 0x8004EBA8.
