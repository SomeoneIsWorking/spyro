...
- PROGRESS 2026-08-06 (C167): parts (1) and (2) are DONE game-side, and the FIRST PRODUCER of part (3) has landed.
  ...
  * (5) THIRD PRODUCER, input/codec/projection slice: game/render/fx_paired_actor.cpp resolves all three 0x80023AC4 animation layers and their guest /16 pose blend, then projects the 238 resolved vertices through a side-effect-free fixed-point RTPS evaluator over the invocation's explicit DR0/1 and CR0..7/24..26 snapshot. game/render/paired_actor_decode.cpp owns the pure normal-stream primitive/material/OT decoder. The exact pre/post-GTE oracle proves cumulative boundaries 100/222/238 (counts 100/122/16 plus three warm-ups), layer-0 stream direction, arithmetic negative-Y borrow, and projection: 384 live invocations each report `target_rtps=241/241 xyz=238/238 projected=238/238 mismatches=0`. This replaced a falsified speculative `GTE_Instruction` replay whose second invocation mismatched 235/238 and fired global hooks. It emits zero faces and the normal handler still refuses state 2.
- NEXT, in the abort's own order: (a) add normal face emission and RenderQueue ownership for 0x80023AC4; dynamically reach and verify its plane-clipping arm before calling the producer complete (resolved XYZ and SXY/SZ are exact); (b) stage 13 modes 1 and 2 of 0x8007CEE4 (page-driven text screens, 3-slot save screen) — RE'd but never reached by a drivable run; (c) the 3D backdrop shared by stage 13 and the FIELD: 0x800521C0, 0x8001F158, 0x8001F798, 0x800258F0, 0x8004EBA8.
