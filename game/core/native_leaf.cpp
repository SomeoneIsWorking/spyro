...
//   * a1 advances by 16 at the TOP of every pass, before any store. If the copy ends mid-pass, a1 is
...
void copy_words_native(Core* c) {
  ...
  for (;;) {
    w[0] = c->mem_r32(src + 0); w[1] = c->mem_r32(src + 4);
    w[2] = c->mem_r32(src + 8); w[3] = c->mem_r32(src + 12);
  ...
  c->r[4] = dst; c->r[5] = src; c->r[6] = end;
  c->r[10] = w[0]; c->r[11] = w[1]; c->r[12] = w[2]; c->r[13] = w[3];   // t2..t5
}

void copy3_owned(Core* c) { ndiff_run(c, "copy3@0x80017700", copy3_native, gen_func_80017700); }
void copyw_owned(Core* c) { ndiff_run(c, "copyw@0x80016958", copy_words_native, gen_func_80016958); }
