...
// root. The two arms are mirror images — whichever of dx/dy is SMALLER is replaced by (3*min)>>3
// and the larger is left alone, then the two are added.
//
...
// ── 0x800168DC — link a node into the display list. 16 static callers, and it writes a 24-BIT
// pointer.
//     v1 = [0x8007581C] ; at = [v1] ; beq at,zero,EMPTY ; sw a0,0(v1)
//     sh a0,0(at) ; srl a0,a0,16 ; jr ra ; sb a0,2(at)      EMPTY: jr ra ; sw a0,4(v1)
// [0x8007581C] points at a small header; its first word is the current head. The new node becomes
// the head UNCONDITIONALLY, because that store is the branch's delay slot. What differs is where
// the OLD head learns about it: an empty list records the node at header+4, a non-empty one writes
// the node's address into the old head's low three bytes — halfword then byte, the PSX display-list
// convention of packing a pointer into 24 bits and leaving the 4th byte for a length or code.
//
