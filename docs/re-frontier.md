...
- where: game/core/frame_loop.cpp (UNCONDITIONAL since 2026-08-17 — the port owns the loop on every run; PSXPORT_SPYRO_FRAME_LOOP retired); game/core/spyro_runtime.cpp SpyroRuntime::bootInit; the render seam it calls is game/render/render_frame.cpp
...
- PROGRESS 2026-08-06 (C167): parts (1) and (2) are DONE game-side, and the FIRST PRODUCER of part (3) has landed.
  ...
  * (3a) MODE-1 BOOT-TO-PLAY RE SLICE, live-proven for the reached corpus (issue 0085, state S002): `game/render/title_menu_recipe.*` transcribes 0x8007CEE4's memory-card-menu switch into the existing 0x8007CD38 native submitter; `title_menu_state.*` is the shared guest-state lens and `title_menu_oracle.*` compares the retained overlay's actual ordered emitter arguments against that same production recipe. At framework pin `99a42aa3`, `scratch/logs/title-menu-reference-99a42aa3.log` records 35 `[titleoracle] PASS` results across reached substates 0, 15, and 1, zero `DIVERGES|REFUSED|STUCK|FATAL` matches, and a clean 3,000-present cap. `scratch/logs/title-menu-native-99a42aa3.log` identifies build `d4de2a1-dirty+psxport-99a42aa3`, emits five consecutive substate-0 frames with `recipe=3 emitted=3`, opens `scratch/saves/card.mcr`, and ends with watchdog signal 06 / libc `abort`. GDB falsified the initial segfault diagnosis and assigns that abort to the explicit mode-2 refusal in `SpyroRenderer::drawFrame`, not a mode-1 failure. Hermetic coverage still owns the unreached switch inputs, but those inputs remain real-corpus gaps.
...
- NEXT, in the abort's own order: (a) own and exact-compare mode 2 of 0x8007CEE4, the now-reached three-slot save screen (issue 0086, state S003); (b) find and drive the writer/scene that makes the 0x80023AC4 parser-control high byte nonzero, then oracle and own the separate alternate/status-plane clipping arm; (c) continue direct ownership of the 3D backdrop. `0x8004EBA8` now has an atomic direct F3/G3 PainterObject producer: its generated-leg oracle matched 8,368 faces and all 11,800 guest N+1 vertex iterations across 17 calls, including source order, projection/content and a forced-corruption negative; a fresh 4100-present native run emitted 876,293 faces across 1,557 nonempty frames with no guest packet/OT writes, no refusal/FATAL and `unscoped-native 0`, while 275 valid-empty calls remained empty. Terrain and paired identities are distinct, but the live corpus reached them on adjacent frame intervals; same-frame multi-object composition remains framework-test evidence, not a Spyro corpus observation. Pool exhaustion remains unseen. The remaining backdrop functions are `0x800521C0`, `0x8001F158`, `0x8001F798`, and `0x800258F0`.
  ...
  - FIELD ENVIRONMENT INVOCATION OWNED, GEOMETRY STILL OPEN 2026-08-22: `field_environment_recipe` is the exact pure branch contract of `0x8002B9CC`. A retained-body boundary oracle matched 523/523 real FIELD calls, including the zeroed 0x1C00 edge-work area and one world call per layer; 1,555 foreign flat-list world calls remained separately counted (`scratch/logs/gate-boot-20260822-181019.log`, gate 13 PASS, framework `ad5cf802`). Feeding that invocation into the existing world recipe exposed the next exact blocker rather than completing the layer: on the first FIELD occlusion-group call, the final-stream oracle matches 645 records and then differs at record 646 only in the U coordinates of a near-subdivided GT4. Count, order, family, SXY, RGB, V, CLUT, TPAGE, material flags, and OT identity agree (`scratch/logs/gate-boot-20260822-173933.log`, issue 0077). No FIELD submitter is wired while that binary-derived UV rule is unresolved. THE FIELD'S REMAINING PRODUCER WORK IS: first correct this near-quad texture reconstruction, then the actor pass's uncovered secondary/shadow/effect families, the cyclorama layer's other COP2 sites 0x80016D2C/0x80050240/0x8004F4BC, PARTICLES 0x800573C8, TRACERS 0x800189F0, and the small 2D layers.
...
- notes: 0x800258F0 RenderWorldChunks IS NOW OWNED in two independent forms. The byte-exact body remains a generated-source/execution oracle (C203), while the shipping semantic producer reads persistent world state and submits immutable LQ, HQ-direct, medium, near and adaptive faces without running that body or consuming packet/OT/GTE output. C215 corrects the old “special-surface” label: the final arm is adaptive GT3/GT4 subdivision and in-place OT replacement. The final-stream oracle compares only packet-observable count/order/family/SXY/RGB/UV/material/bin; the shipping pure-projection seam proves SZ/view-Z separately. The moving corpus `gate-boot-20260822-153121.log` matched 1,275 changing calls over 3,000 frames with required negative controls. Active world animation still refuses the semantic recipe, so that state is the next honest dependency rather than a stale/static fallback.
