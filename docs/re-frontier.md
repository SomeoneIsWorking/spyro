...
- NEXT, in the abort's own order: (a) dynamically reach and own the separate alternate/status-plane clipping arm, then define stable per-frame topology identity and temporal endpoint snapshots for the now-float normal group before wiring its pure projection rerun into the framework FPS60 world pass. Never rebuild descriptors or read changing guest selectors at present time. (b) stage 13 modes 1 and 2 of 0x8007CEE4 (page-driven text screens, 3-slot save screen) — RE'd but never reached by a drivable run; (c) the 3D backdrop shared by stage 13 and the FIELD: 0x800521C0, 0x8001F158, 0x8001F798, 0x800258F0, 0x8004EBA8.
