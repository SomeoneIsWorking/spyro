...
This file is the repository-local entry point and the single game-specific source of truth.
`CLAUDE.md` is a SYMLINK to it, so both names carry these exact contents and no copy can drift out of
step with the other. Two authorities sit above it and are not repeated here:

1. [`../AGENTS.md`](../AGENTS.md) — workspace-wide rules and the repository-lineage decisions.
2. [`external/psxport/AGENTS.md`](external/psxport/AGENTS.md) — the shared framework contract.
...
**A deterministic repro lives in a pad recording — copy it out of the rotation before using it.**
`PSXPORT_PAD_RECORD` is default-on for windowed runs and rotates `scratch/bin/pad_session.pad` five
deep, so the recording that reproduces a bug becomes `.1`, then `.2`, and is eventually dropped. A
replay that silently drove a different session is indistinguishable from a fixed bug. Copy the file
to a named path the moment it matters (`scratch/bin/issue89_newgame_route.pad` is this issue's) and
set `PSXPORT_PAD_RECORD=0` on replays. The `[padrec] replaying N frames` line names the frame count
— check it is the one you meant.

**A refusal fatal dumps RAM, so a stage-0 layer can be worked on from a captured frame.**
`SpyroRenderer::abortUnimplemented` calls `snapshot_now` unconditionally, for the same reason the
recomp-MISS path does: that 2 MB image is what the next question gets answered from. The reference
leg cannot reach the field (it fail-fasts at the title overlay's guest-VSync tail by design), so a
captured frame is the ONLY way to run a retained-body oracle against FIELD state.
