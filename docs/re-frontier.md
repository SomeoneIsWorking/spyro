...
- evidence: C127,C129,C130,C136,C137,C138,C139,C140,C141,C176,C177,C198
...
- notes: RenderWorldChunks is structurally mapped (C198): phase 1 culls the chunk list by view frustum+distance against m_ViewMatrix and applies environment animation; phase 2 RTPS-projects 11/11/10-bit packed-delta vertices against m_ProjectionMatrix, caches SXY2+SZ3 in the scratchpad (0x1F800000, 8-byte stride) and emits gouraud quads/tris (NCLIP + depth-LOD + clip-code reject + OT bin by average SZ); phase 3 emits the special-surface (water/lava) quads from D_8006D5C8. DECISIVE DEPTH FACT: per-vertex SZ3 is a GTE/scratchpad intermediate used ONLY for LOD + OT binning and is NEVER written into the emitted packets (SXY+color only) — the reason an interpreted body cannot supply depth. Clip-bound immediates 0x10000/0x1000000/0x2000000 load at 8 sites (0x80026264, 0x80026844, 0x80027A58, 0x80028174, 0x80028C48, 0x800299A4, 0x8002A2EC, 0x8002A4B8). Next: a live input census of 0x800258F0 (the PSXPORT_SPRITE_QUEUE_CENSUS pattern in native_render.cpp) to measure chunk/vertex/face counts and the SZ3 range before oracle work. RasterizeSpritePrimQueue 0x80022A2C is measured at its game-state input boundary across all four variants (C176). Its screen-space flat-bit01 subset now matches the guest's exact candidate/accepted face census at stage-13 timer 171; world-space records and the other three variants still need visibility, transforms, colour/depth and queue emission owned. C177 remains falsified and is not evidence for this result.
