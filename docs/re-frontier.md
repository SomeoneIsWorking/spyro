...
- PROGRESS 2026-08-06 (C167): parts (1) and (2) are DONE game-side, and the FIRST PRODUCER of part (3) has landed.
  ...
  * (5) THIRD PRODUCER, input/codec/projection/normal-face slice: game/render/fx_paired_actor.cpp resolves all three 0x80023AC4 animation layers and their guest /16 pose blend, then projects the 238 resolved vertices through a side-effect-free fixed-point RTPS evaluator over the invocation's explicit DR0/1 and CR0..7/24..26 snapshot. game/render/paired_actor_decode.cpp owns the pure normal-stream decoder and hermetic acceptance/content resolver. The exact pre/post-GTE oracle proves cumulative boundaries 100/222/238 (counts 100/122/16 plus three warm-ups), layer-0 stream direction, arithmetic negative-Y borrow, and projection: 384 live invocations each report `target_rtps=241/241 xyz=238/238 projected=238/238 mismatches=0`. Generated-code transcription plus discriminating tests now cover triangle NCLIP/two-sided acceptance, the complete quad sign table and GT3 diagonal substitutions, compact RGB/attribute/opcode content, signed raw-depth rejection, and stable descending/FIFO-tie ordering; the content comparator reports the first field and both denominators. This replaced both the accept-all face resolver and a falsified speculative `GTE_Instruction` replay. **Not yet verified live:** numeric OT-bin equality and the final splice relative to packets already in the global OT. It emits zero native faces; alternate/status-plane clipping is deliberately outside this normal resolver.
- NEXT, in the abort's own order: (a) prove numeric bins and global-OT splice on live packets, then add normal face emission and RenderQueue ownership for 0x80023AC4; dynamically reach and verify the separate alternate/status-plane clipping arm before calling the producer complete (resolved XYZ and SXY/SZ are exact); (b) stage 13 modes 1 and 2 of 0x8007CEE4 (page-driven text screens, 3-slot save screen) — RE'd but never reached by a drivable run; (c) the 3D backdrop shared by stage 13 and the FIELD: 0x800521C0, 0x8001F158, 0x8001F798, 0x800258F0, 0x8004EBA8.
