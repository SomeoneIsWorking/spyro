...
| Frame lifecycle | Finite native boot and JIT update steps, display-field callbacks, cadence, and one presentation fence | `titles/spyro1/core/spyro1_{boot_sequence,field_scheduler,frame_driver}.*` | `Spyro1FrameDriver`, `FieldScheduler::deliver` | `docs/re-frontier.md` |
...
| World geometry | Source capture, codec, projection, LQ/HQ/refinement recipes, animation, scene building, and queue submission | `game/render/world_*`, `fx_world_draw.*`, `fx_field_environment.*` | `spyro_world_submit`, `spyro_field_environment_submit` | `docs/findings/world-semantic-oracle.md` |
...
| Temporal scene integration | Per-frame source eligibility, exact producer membership, and paired endpoint reconstruction/rotation through the shared presenter | `game/render/temporal_scene.*` | `spyro_temporal_scene_prepare`, `spyro_temporal_scene_source` | `docs/project-state.md` |
