...
- gap: 
- notes: PHASE DONE: the high-caller LEAF work is complete. tools/own_candidates.py now derives 24 owned bodies from the live ndiff_run sites; C081's historical count of 15 is superseded. Four are GTE bodies, owned without reimplementing the GTE — scalar logic native, COP2 via the platform's gte_op/gte_read_data/gte_write_ctrl, integer divide via cpu_div (C++ division is UB exactly where MIPS defines behaviour: /0 and INT_MIN/-1). Its best remaining LEAF has 15 callers, down from 136, so this seam has given what it has to give. CAVEAT on exhausted: the caller count is STATIC, so indirect calls are invisible and a low count is not proof of coldness — a runtime call histogram is the honest measurement. Continuing is tracked by own.non-leaf.

### own.non-leaf — Own NON-LEAF functions, bottom-up from the leaves already owned
- status: re-partial
- deps: own.next-targets
- evidence: C082,C207,C209,C210,I019,I020,I028
- where: game/core/native_printf.cpp; game/core/native_actor_mesh_scratch.cpp; game/core/native_spu_pio_upload.cpp; game/core/spu_pio_upload.h; tools/own_candidates.py --ready-nonleaf
- gap: THIRD STEP VERIFIED, not the family. PsyQ WriteSpuRamPio 0x8005BE88 is now owned bottom-up: 115 instructions from SCUS_942.28, only the already-owned spin60 and printf children, and one real frame-zero SPU-init call that matched the retained generated body exactly under PSXPORT_NDIFF=1. The focused test covers zero, odd, full 0x40-byte, and multi-batch boundaries. The authoritative ranker now filters dependency-ready non-leaves to generated MAIN entries; issue 0070 records the three false jr-ra boundaries it previously admitted. Continue by expanding the same executable-derived filter only when all smaller real candidates are proven cold, then require FNTRACE reach before implementation.
- notes: C082 puts all guest code at only 4.5-4.9% of host CPU, so further ownership buys correctness and architecture, not speed. Pick from tools/prof_hot.py for measured speed work and from the static queue for coverage. FNTRACE and an owned override cannot be armed on the same address because both use its one override slot; C207, C209, and C210 therefore cite separate reach and equality runs.
