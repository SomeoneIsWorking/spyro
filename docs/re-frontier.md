...
- notes: The field producer order is wired in game/render/render_frame.cpp. The framework line-list path is an explicit primitive owner, not a triangle approximation. A paced audio run is non-silent and 20.02 seconds for 1200 VBlanks, but reports XA ring-full back-pressure; SBS oracle boot exits cleanly to 120 fields but retains five stack-only differing bytes and one unreached owned address.
